...
- 状态：草拟；设计体系与首发主题已确认，其余列明事项待确认。
- 日期：2026-08-27。
- 当前事实：用户具备 ComfyUI 等静态生图能力，暂不具备视频制作能力；M1 Home、Collection、Entry、Review Board、3×2 探针与字体验收页已冻结为不可发布的工程参考基线，当前没有应用源码、应用依赖或运行环境。用户明确表示现有页面风格尚未达到预期，人工视觉满意度未通过；冻结不等于设计批准。
...
- affiliate、sponsor、ad、paid product 使用独立字段和可见披露。
...
- 网站技术栈在 M2 应用初始化前确认；托管在首次远端预览前确认；邮件、分析和表单服务在各自集成里程碑前确认。
...
- 是否最终确认 Astro + Markdown/MDX 静态核心、Next.js 条件式重评、Vercel 首选/Cloudflare 备选的路线。

...
- 状态：草拟；设计体系、首发主题与 M2 技术/内容方向已确认，M2 本地实施已完成，其余列明事项待确认。
- 日期：2026-08-27。
- 当前事实：用户具备 ComfyUI 等静态生图能力，暂不具备视频制作能力；M1 Home、Collection、Entry、Review Board、3×2 探针与字体验收页已冻结为不可发布的工程参考基线。M2 已按静态优先、内容合同优先且不提前建设商业能力的方向完成本地应用、依赖、Content Layer 与工程门禁；这些中性调试页不是生产视觉，也未启动服务或发布。用户明确表示现有 M1 页面风格尚未达到预期，人工视觉满意度未通过；冻结与 M2 工程完成都不等于设计批准。
...
验收：先核准适用的 claim、术语和视觉 brief，再开始视觉制作；每张 approved master 有工具无关的 sidecar manifest，参考图、人工修改、公开导出物、accessibility mode 和权利状态可追溯；纯装饰 invented 背景不得支撑事实。只有实际使用 ComfyUI 或其他生成工具时才记录其模型与工作流 metadata。
...
- M2 技术与内容合同、固定运行时、依赖安装和本地应用初始化已经完成；dev/preview 服务、M3/M4 实施与任何版本控制写入仍不由 M2 完成自动授权。托管在首次远端预览前确认；邮件、分析、表单与未来商业服务在各自独立里程碑前确认。

### 主要风险与缓解

- **文化层次被混淆或空白被生编补齐。** 使用固定传统类型、声明级证据和编辑模板；无可靠证据时保留未决或不发布。
- **英文像翻译腔或关键术语被错误一对一替换。** 使用语境化术语记录；先完成双语核对，再由母语编辑或目标读者测试可读性。
- **图片很漂亮但不准确。** 视觉元素拆成 canonical / inferred / invented，并执行文化审校。
...
- **Git 被原图拖垮。** 代码仓库只保存审核后的 Web 资产及 manifest；大原图和模型外置。

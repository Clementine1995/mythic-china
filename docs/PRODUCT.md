...
- 状态：草拟；设计体系、首发主题与 M2 技术/内容方向已确认，其余列明事项待确认。
- 日期：2026-08-27。
- 当前事实：用户具备 ComfyUI 等静态生图能力，暂不具备视频制作能力；M1 Home、Collection、Entry、Review Board、3×2 探针与字体验收页已冻结为不可发布的工程参考基线。M2 已确认静态优先、内容合同优先且不提前建设商业能力的技术方向，但当前仍没有应用源码、应用依赖或项目运行环境。用户明确表示现有页面风格尚未达到预期，人工视觉满意度未通过；冻结不等于设计批准，决策确认也不等于 M2 已实施。
...
- M2 技术与内容合同已经确认；项目运行时切换、依赖安装和应用初始化仍需单独授权。托管在首次远端预览前确认；邮件、分析、表单与未来商业服务在各自独立里程碑前确认。

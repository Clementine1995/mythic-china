...
- 已确认第二 Collection 方向：工作名为 `Strange Tales from Liaozhai`，`Strange Tales After Dark` 只保留为可选营销副标题；六篇采用 007 的 3+3 分配与两条公开阅读顺序，Zhong Kui 继续作为 Featured。Project owner 已确认 Liaozhai 的任 2016 主/张 2011 对校研究路线和《促织》青柯亭单见证路线；当前四个新增 draft 均有证据受限首稿。Painted Skin 先采用已核上海图书馆 1766 青柯亭本，任/张比较与现代译本/影视的直接比较延期。第二 Collection 与两个三篇成员路径已按 009 建立；Liaozhai 的资产、Featured 与状态提升仍未完成。
...
本节目前只有创作者经验和项目设想，没有用户研究数据。四篇英语草稿的修订前形成性审核包已被当前版本取代；Project owner 聚焦双语确认后，协调者 Markdown 与唯一参与者 HTML 已按 [`010-four-entry-reader-review.md`](requirements/010-four-entry-reader-review.md) 从当前 build 重生成并通过静态检查。真人反馈仍为 0，不能写成用户研究完成。当前参与者 HTML 须先由 1 位不计入正式样本的目标读者试跑，再由 5–8 位正式目标读者完成文稿审核；MVP 上线前还须在最终制品上测试首页定位、英文标题、文章模板和来源说明，上线后以真实行为数据替代本节假设。

...
用户已经确认“中国神话传说博物馆”的系统方向，以及“中国阴间 + 钟馗 Entry”的首个纵切片，并于 2026-08-27 明确要求冻结 M1 工程参考，同时明确当前页面风格并非期望结果。因此现有原型只作为不可发布的工程/结构基线冻结：阅读顺序、链接、响应式与无障碍骨架可以进入后续实现参考，人工视觉满意度未通过，页面外观、字体落地与美术表达不得写成已批准。后续表现层可沿既定替换缝重做，不得改写内容、证据、稳定身份、URL 与语义合同。
...
- 首个真实 Home、Collection、Entry 纵切片必须再次提交用户确认美术方向；若仍不符合目标，应只重构共享表现层，不改写内容、证据关系、稳定身份、URL 或语义阅读顺序。

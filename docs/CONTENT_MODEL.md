...
- 当前内容 inventory 为 6 篇 Entry（2 篇 `editorial-review`、4 篇 `draft`）、1 个 `editorial-review` Collection、14 份 Source、19 份 Claim 与 5 份 Terminology；published Entry / Collection 仍为 0/0。四个新增 Entry 的稳定身份为 `ten-kings`、`liaozhai-reading-guide`、`painted-skin` 与 `fighting-cricket`；Ten Kings 已有正文及 2 Source / 3 Claim / 1 Terminology 反向消费关系，Fighting Cricket 已有正文及 1 Source / 3 Claim / 1 Terminology 反向消费关系，Liaozhai 导读与 Painted Skin 的正文和反向数组仍为空。四篇均处于研究/写作阶段的 `draft`，不是可发布状态。

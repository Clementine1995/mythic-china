...
本节保留 M1 的比较问题与历史命名。第 1–4、7 项继续作为设计审查证据；第 5–6 项现按 [`011-public-beta-validation.md`](requirements/011-public-beta-validation.md) 作为 Public Beta 上线后的目标读者研究问题。缺少样本或暂未达到阈值不会阻塞内容状态、public artifact、受保护预览或 Public Beta 生产发布；研究暴露的事实、来源、文化安全、无障碍或视觉缺陷仍回到对应硬门禁处理。
...
M1 执行记录（2026-08-27）：`prototypes/m1-review-board.html` 已把四个第二轮系统分别落实为“借用目标 → 落点 → Mythic China 转译 → 不采用皮肤”，并保留首轮台账、3×2 家族探针、无图/灰度和 30 秒可信度检查入口。用户随后明确冻结 M1 以停止继续扩展设计研究，同时指出当前页面风格仍不是目标方向；因此 M1 的比较证据交付已收口，差异化观感和目标读者可信度没有被记录为用户满意或正式读者测试通过。当时记录的“首个真实纵切片复核”已由 011 的当前路线取代：第 5–6 项在 Public Beta live artifact 上随 R2a/R2b 执行。

...
| 实施状态 | M1 已冻结；M2、M3、M4 本地产品实现、M5-U2/U3、四篇证据受限草稿、两个合集路径、010 的文案与 R1，以及 012 的五组 Hero 资产闭环已完成；U4、关系/Featured/状态、M6 public artifact、M7 Public Beta 与 R2 未完成 | 三个 provider-neutral service/Fake、inert review UI/Privacy、6 Entry / 2 Collection / 14 Source / 25 Claim / 6 Terminology 与 14 页 review inventory 已建立。六份 Terminology 均为 `bilingual-approved`，Painted Skin 短译 Claim 为 `verified`。当前 6 篇 Entry 与 2 个 Collection 均绑定自己的 approved/current Hero；五组新资产已有 master、production record、manifest、repository source 和 versionless 内容外键。修订前 R1 已失效；重建 R1 已退役为 reference-only，只保留问题、rubric 与文稿基线，R2a 等待 Public Beta live artifact。四篇 `relatedEntryIds`、Liaozhai Featured、状态、真实供应商 transport、public artifact 与生产仍未实施 |
| 验证状态 | M1 工程/结构证据保留；M3/M4 适用证据通过；M5-U2/U3 完整本地门禁通过；当前内容、Schema、模板、output、视觉生产链与 R1 静态门禁通过，真人验证未开始 | 五组 Hero 数据闭环的 8 文件/38 项定向测试通过；非默认视觉入口实际核验 21 个 local master、17 份 current responsive rendition 与 120 个输出。最终完整工程门禁通过 Prettier、ESLint、25 文件/369 项测试、Astro 81 文件零诊断、14 页/112 Hero/10 WOFF2/零 XML/零客户端 JavaScript；14 页/42 Hero 是 2026-09-06 历史基线。当前 R1 的 20/20 输入哈希、4 篇可见文本、结构、离线性、唯一 ID/ARIA/片段引用与严格 UTF-8 检查通过；真人反馈、新页面浏览器、外部服务、M7 live/RUM 与 Public Beta R2 未验证 |
| 发布状态 | 未发布 | 未来托管目标为 Vercel；当前不购买自定义域名，且未创建项目、稳定 production hostname、真实 origin、部署配置或 public artifact |

- 当前权威结论更新时间：2026-09-07。
- M4-U3 实现与原收口文档已由 Project owner 提交为 `e94eacaad989652c7f71ae50276652cc3f54997a`（`updatee`）；M4-U2 历史基线为 `5f327b63f7a227e54773718d140e7295ef6ed3c9`（`M4-u2 completed`），M3/M2 历史基线为 `c606f5a` / `f258227`。Project owner 后续把 M4 完成状态提交为 `3983bee91ada4a286613ec702a8009a4f528af3f`，又把 M5-U1–U3、U4 准备快照与 007 提交为 `3fa46d5f85c43a5278e15ca6b0630d724439acc9`。四篇研究、证据最小物化及相关状态同步提交为 `9914dd304467118b2c32f7f8ac192cc00344fb92`；Ten Kings、Fighting Cricket 与 Liaozhai Reading Guide 分别形成后续检查点。009、四篇当前草稿与 010 当前审核基线已进入本地 HEAD `f81b49fd1c288aed6c552c2e8d9926ec5a09e86c`，Public Beta 路线合同进入 `96da8db52cc384e0dc77164288d3bce16383faf0`。本轮五组 Hero 从该基线开始；当时本地 HEAD、`main` 与本地 `origin/main` tracking ref 对齐且工作树干净。本轮未 fetch，因此不以 tracking ref 证明服务器端状态；当前存在 012 直接链路的内容、视觉资产、模板、样式、验证和文档修改，未执行 Git 写操作、建分支或 worktree。
...
- 推荐工程顺序：M4 已闭合；M5-U2 provider-neutral 合同/Fake 与 U3 inert review UI/Privacy 已实现，U4 仍未完成。第二 Collection、3+3 分配、四篇证据账本、证据受限草稿、两个合集路径及五组 Hero 已完成；修订前锁定稿、当前 AI 专业修订单元与两条 `contentNote` 均已通过双语确认，R1 已重建、通过静态检查并退役为 reference-only。内容下一停点是四篇 `relatedEntryIds`、Liaozhai Featured 与状态审核，再由 M6 完成 6 篇可发布内容包、至少 2 个 Collection 与逐项 `ready`，由 Project owner 作出 `published` 决定；HTTPS origin、public artifact、clean-source QA、预览与 M7 Public Beta 生产/live/RUM 均继续逐次授权。Public Beta 上线后执行 1 位非计入样本的 R2a；R2b 建立 5–8 份核心全站可用记录与四篇各前 5–8 份可用深读，按预冻结规则可补位，处置结果后关闭正式 MVP 验证。
- 当前动作边界：本地 HEAD 基线为 `96da8db`，本轮开始时工作树干净；当前修改只覆盖 012 的五组 Hero 直接链路。6 个 Entry / 2 个 Collection 均有对应关系和自己的 approved/current Hero，四个新增 `draft` Entry 均有证据受限正文；六份 Terminology 为 `bilingual-approved`，Painted Skin 短译 Claim 为 `verified`。本批不处理 `relatedEntryIds`、Liaozhai Featured、状态、依赖、服务、供应商账户/action/link、真实写入、public/Vercel、远端预览或发布，也未执行 Git 写操作。
...
- 首批四个新增选题已形成原典/对象、学术解释与关键术语的候选 map，并完成当前可取得见证、正式书目与 locator 的分层闭合；证据子集已物化，四篇均有精确消费清单、证据受限草稿和自己的 approved/current Hero。修订前锁定稿及当前改动单元均已通过 Project owner 双语确认；四篇仍待 `relatedEntryIds`、状态和新页面级发布候选 QA，目标读者研究在 Public Beta 上线后执行。Ten Kings 的两个见证不能互作转录；Fighting Cricket 与 Painted Skin 的单一青柯亭见证不能外推为跨见证或作者归属结论；Guide 不能把尚未取得实际册页的任/张版本写成已核来源。
...
4. 四篇 `relatedEntryIds`、Liaozhai Featured、最终内容状态和页面级发布候选 QA；当前修订单元、两条内容提示与五组 Hero 资产级审核已完成。两个 Collection 对象/`entryIds` 已建立，但仍不降低“至少 2 个完整合集页”的 M6 预览门槛。目标读者审核按 011 在 Public Beta 上线后执行。
...
- 实施状态：M1 已完成并冻结为工程参考基线；M2、M3、M4 本地产品实现、M5-U2/U3、四篇证据受限草稿、两个合集路径、010 的文案/R1 和 012 的五组 Hero 资产闭环已完成。目标读者 R2、U4 的真实 provider transport/联调、U5、四篇 `relatedEntryIds`、Liaozhai Featured、内容状态、public artifact assembly 与 M7 尚未完成。
- 验证状态：M3/M4 与 M5 历史证据保留。五组 Hero 数据闭环的 8 文件/38 项定向测试通过，非默认视觉入口已实际核验 21 个 local master、17 份 current responsive rendition 与 120 个输出；最终完整工程门禁通过 Prettier、ESLint、25 文件/369 项测试、Astro 81 文件零诊断、14 页/112 Hero/10 WOFF2/零 XML/零客户端 JavaScript。当前 R1 的 20/20 输入哈希、4 篇可见文本、结构、离线性、唯一 ID/ARIA/片段引用与严格 UTF-8 检查通过；目标读者、新页面人工浏览器审核、真实 public output、供应商行为、键盘/缩放、偏好/故障模式、性能、实际 fallback face 与支持平台仍未验证。
- 发布状态：未发布。
- 已满足：项目范围、M2 目标架构与内容合同、引用/资产边界、实施拆分，以及“中国神话传说博物馆 + A 主 C 辅 Home 概念方向 + 中国阴间 + 钟馗 Entry”的工程参考与表现层替换边界已形成；未来商业化只保留隔离出口，不进入 M2 实现。
- 尚未满足：四篇 `relatedEntryIds`、Liaozhai Featured、内容状态和新页面级最终人工审校；M5-U4 的 Buttondown 账户级配置/真实 action/合成订阅联调、Tally 草稿发布与真实联调、U5 Plausible hook，以及真实 public origin、M6 public runner/Layout metadata/endpoint、最终 release-candidate QA、verification receipt、Vercel 项目与部署等后续门禁。任笃行/张友鹤实页、CText/Giles 直接比较、现代译本与影视证据已移出当前 MVP，只有未来恢复相应比较时才重新成为门禁；青柯亭 `業魅`、Tso `孽魅` 与 CText 关联未具名见证 `孽鬼` 的跨见证来源和演变也未解释。真实键盘/200%/偏好、慢/阻断加载、图片失败、LCP/CLS、实际 fallback face、跨平台 fallback，以及最终 inventory 的页面级人工批准尚未闭合。目标读者反馈为 0，Public Beta 上线后仍须完成 R2a/R2b、问题处置、复测与 Project owner 收口。未验证项没有被改写成通过。本期未使用 ComfyUI，因此按 M3 合同不需要 workflow/model registry。
- 8 个 Hero family、Terminology 双语/CJK 门禁、M4 历史页面与人工判断已闭合各自范围；M5-U2 provider-neutral 合同/Fake 与 U3 inert review UI/Privacy 已落地。M6 内容方向、四篇研究/证据物化/草稿与两个合集路径已经建立；修订前英文批准及 010 当前改动单元的聚焦双语确认均已完成，当前 R1 已重建并通过静态检查。下一内容停点是四篇 `relatedEntryIds`、Liaozhai Featured 与状态审核；R2a/R2b 在 Public Beta 上线后执行。本批不自动授权关系、真实联调、内容状态提升、Git 提交/push、public/Vercel、远端预览或发布。
- 是否可以关闭需求：否。

...
| 维度 | 当前状态 | 证据或阻塞项 |
| --- | --- | --- |
| 需求状态 | 草拟；M1、M2、M3、M4 与 M5-U2/U3 已完成，M5-U4 账户准备事实已同步；第二 Collection 方向、Liaozhai 工作底本路线、《促织》单见证路线、四篇 claim map/证据账本、证据最小物化及 Ten Kings / Fighting Cricket / Liaozhai Reading Guide 三篇内容纵切片已确认并实施；U4/U5、Painted Skin 正文、资产/Collection 关系/状态与 public artifact assembly、M7 待后续门禁 | M4 详细合同见 [`003-pages-exploration-seo.md`](003-pages-exploration-seo.md)；M5 边界见 [`006-external-interactions.md`](006-external-interactions.md)；第二 Collection 决策与四篇研究、物化及三篇纵切片分别见 [`007-second-collection-decision.md`](007-second-collection-decision.md)、[`008-four-entry-claim-maps.md`](008-four-entry-claim-maps.md) |
| 实施状态 | M1 已冻结；M2、M3、M4 本地产品实现、M5-U2/U3、四篇证据最小物化与三篇证据受限首稿已完成；U4 未完成 | 三个 provider-neutral service/Fake、inert review UI/Privacy、4 个 draft Entry owner、5 Source、9 Claim、3 Terminology、Source 数字影像证据门禁与 13 页 review inventory 支持已建立；Ten Kings 已绑定 2 Source / 3 Claim / 1 Terminology，Fighting Cricket 已绑定 1 Source / 3 Claim / 1 Terminology，Liaozhai Reading Guide 已绑定 1 Source / 2 Claim / 1 Terminology，三篇均保持 `draft`，只有 Painted Skin 仍为空。图片、Collection/related 关系、状态、真实供应商 transport、public artifact 与 M7 生产均未实施 |
| 验证状态 | M1 工程/结构证据保留；M3/M4 适用证据通过；M5-U2/U3 完整本地门禁通过；本轮内容/Schema/output 完整门禁通过 | Liaozhai Reading Guide 纵切片后，完整 Prettier/ESLint/25 文件/321 测试/Astro 81 文件零诊断/13 页 build/output verifier 通过；42 Hero、10 WOFF2、零 XML/客户端 JS。首次命令在无断言失败前遇到 Vitest worker 意外退出，未改文件后原命令重跑通过；浏览器、外部服务与 M7 live/RUM 未验证 |
| 发布状态 | 未发布 | 未来托管目标为 Vercel；当前不购买自定义域名，且未创建项目、稳定 production hostname、真实 origin、部署配置或 public artifact |

- 当前权威结论更新时间：2026-09-05。
- M4-U3 实现与原收口文档已由 Project owner 提交为 `e94eacaad989652c7f71ae50276652cc3f54997a`（`updatee`）；M4-U2 历史基线为 `5f327b63f7a227e54773718d140e7295ef6ed3c9`（`M4-u2 completed`），M3/M2 历史基线为 `c606f5a` / `f258227`。Project owner 后续把 M4 完成状态提交为 `3983bee91ada4a286613ec702a8009a4f528af3f`，又把 M5-U1–U3、U4 准备快照与 007 提交为 `3fa46d5f85c43a5278e15ca6b0630d724439acc9`。四篇研究、证据最小物化及相关状态同步提交为 `9914dd304467118b2c32f7f8ac192cc00344fb92`；Ten Kings 纵切片随后进入 `a5a15a5c74f7a18a8c15e0cdd5ac1e44574151c2`，Fighting Cricket 纵切片在中文概要确认后进入 `7c0cb3fbea2c6175f01790160fe2460bb658e0f8`。Project owner 随后推送该检查点；本批开始时 HEAD、`main` 与本地 `origin/main` tracking ref 对齐。Liaozhai Reading Guide 于 2026-09-05 获中文概要确认与独立本地提交授权后随本检查点进入当前本地 HEAD；本检查点未执行 fetch、push、建分支或 worktree。提交前的本地 `origin/main` tracking ref 仍为 `7c0cb3f`，不单独证明服务器端状态。
...
- 英语优先、单人配合 AI 维护和指标阈值仍需通过真实读者验证；第二 Collection、公开阅读顺序、Liaozhai 任 2016 主/张 2011 对校工作路线与《促织》青柯亭单见证路线均已确认，四篇 claim map、证据最小物化与 Ten Kings / Fighting Cricket / Liaozhai Reading Guide 证据受限首稿已完成。任/张实际册页、译文与文化审校仍有门禁；《促织》跨见证、作者归属与“首次增写”继续排除，且全部新增选题尚未经过目标读者验证。

未验证风险：

- `Mythic China` 已确认为当前站点品牌，公开数据控制者名称为 `hyc`；品牌商标与未来自定义域名可用性尚未核查。
- 首批英文文案、SEO 主题和新视觉执行尚未经过目标读者测试；方向已确认不等于原型效果已验证。
- 首批四个新增选题已形成原典/对象、学术解释与关键术语的候选 map，并完成本轮可取得见证、正式书目与 locator 的分层闭合；证据已闭合子集已物化为 Source、Claim 与 Terminology。Ten Kings、Fighting Cricket 与 Liaozhai Reading Guide 已建立精确消费清单与证据受限首稿，但尚未完成双语、图片、目标读者或状态审核；Ten Kings 的两个见证不能互作转录，Fighting Cricket 的单一青柯亭见证也不能外推为跨见证或作者归属结论，Guide 也不能把尚未取得实际册页的任/张版本写成已核来源。
...
4. Painted Skin 的正文与 Entry 证据消费关系，以及四篇的第二 Collection 对象/`entryIds`、最终公开标题、翻译、视觉资产与状态；Ten Kings、Fighting Cricket 与 Liaozhai Reading Guide 虽已有证据受限首稿，仍须双语/编辑/目标读者审核，也不降低“至少 2 个完整合集页”的 M6 预览门槛。
...
- 结果：M1 工程参考、M2/M3 基线及 M4 本地产品实现已收口。M4 包括 U1/U2/U3、U4A public SEO 纯基础设施、机器门禁、Chinese Underworld Collection/Guide Hero、CJK 静态链、Windows/local 部分预检、review 功能页修正、U5A noindex 字体样张、最终三档基础矩阵及当时 8 页人工视觉判断。M5-U2 又完成 provider-neutral 合同/Fake，U3 完成 inert Newsletter / Reader Request review UI、Privacy 页面和匹配 output oracle，形成 9 页历史基线；本轮四个 draft Entry 通过既有动态路由把当前 review inventory 扩为 13 页。四篇证据最小物化现为 4 draft Entry / 5 Source / 9 Claim / 3 Terminology；Ten Kings、Fighting Cricket 与 Liaozhai Reading Guide 又分别完成 2 Source / 3 Claim / 1 Terminology、1 Source / 3 Claim / 1 Terminology 和 1 Source / 2 Claim / 1 Terminology 的消费关系与证据受限首稿；更早 M4 证据见 `003-pages-exploration-seo.md`，M5 证据见 [`006-external-interactions.md`](006-external-interactions.md)，命令与运行证据以 `../../DEV_WORKFLOW.md` 为准。
- 未完成/风险：M5-U4 的 Buttondown 账户级配置/真实 action/合成订阅联调、Tally 草稿发布与真实联调、U5 Plausible hook，以及真实 public origin、published 内容、M6 public runner/Layout metadata/endpoint、最终 release-candidate QA、verification receipt、Vercel 项目与部署均未建立。两篇 `editorial-review` 候选仍需 Project owner 内容/状态审核；Painted Skin 仍是空 draft。四个新增 Entry 均无资产、Collection/related 关系或状态审核；Ten Kings、Fighting Cricket 与 Liaozhai Reading Guide 还未完成双语、目标读者与页面级人工审核，Guide 的任/张实际册页也未取得。真实键盘/200%/偏好、慢/阻断加载、图片失败、LCP/CLS、实际 fallback face 与跨平台 fallback 尚未闭合。
- 版本身份：Project owner 把 M4 完成状态提交为 `3983bee91ada4a286613ec702a8009a4f528af3f`，后续把 M5 与 007 状态提交为 `3fa46d5f85c43a5278e15ca6b0630d724439acc9`。四篇研究、证据最小物化及相关状态同步提交为 `9914dd304467118b2c32f7f8ac192cc00344fb92`；Ten Kings 纵切片随后进入 `a5a15a5c74f7a18a8c15e0cdd5ac1e44574151c2`，Fighting Cricket 纵切片在中文概要确认后进入 `7c0cb3fbea2c6175f01790160fe2460bb658e0f8`。Project owner 随后推送该检查点，本批开始时 HEAD、`main` 与本地 `origin/main` tracking ref 对齐。Guide 在 2026-09-05 获中文概要确认与独立本地提交授权后随本检查点进入当前本地 HEAD；本检查点未执行 fetch、push 或发布。提交前的本地 `origin/main` tracking ref 仍为 `7c0cb3f`，不单独证明服务器端状态；精确身份仍以执行前只读核查为准。

## 13. 当前最终结论

- 需求状态：草拟；M1 工程冻结决策、设计体系、A 主 C 辅的 Home 概念方向、首个 Collection、钟馗 Featured Entry、M2 技术/内容合同、M4 第 1–9 项、M4-U4A 的未来 Vercel 托管方向/公共身份/text-only OG、M5-U2/U3 合同，以及第二 Collection 方向与 3+3 分配均已确认；U4 账户准备事实、四篇 claim map/分层证据、Liaozhai 工作底本路线、《促织》单见证路线、证据最小物化及三篇纵切片已记录，后置决定仍按对应单元确认。
- 实施状态：M1 已完成并冻结为工程参考基线；M2、M3、M4 本地产品实现、M5-U2/U3、四篇证据最小物化及 Ten Kings / Fighting Cricket / Liaozhai Reading Guide 证据受限首稿已完成。Buttondown 账户审核已通过、Tally 草稿仍未发布；U4 的真实 provider transport/联调、U5、Painted Skin 正文、全部图片/Collection 关系/状态与 public artifact assembly、M7 尚未开始。
- 验证状态：M3/M4 历史工程、视觉与浏览器证据保留；M5-U3 的 3 文件/79 测试、25 文件/320 测试、Astro 81 文件零诊断与 9 页输出保留为历史基线。2026-09-04 Liaozhai Reading Guide 纵切片后的最终完整 Prettier、ESLint、25 文件/321 项测试、Astro 81 文件零诊断、13 页 build/output verifier 已通过；实际输出为 42 Hero、10 WOFF2、零 XML、零客户端 JavaScript，六个 Entry 的 inactive Reader Request 与每页唯一 Newsletter 均由 oracle 验证。第一次执行在任何断言失败前遇到 Vitest worker 意外退出；没有修改文件或降低门禁，原命令重跑后通过。Project owner 提供的审核/账户/草稿状态不是供应商行为验证；三篇新首稿的图片、双语/页面人工审核、任/张实际册页、《促织》跨见证异文、Painted Skin CText/Giles 权利、真实 public output、供应商行为、新页面浏览器/键盘/缩放、偏好/故障模式、本地/预览性能、实际 fallback face 与支持平台仍未验证，归相应后续门禁。
- 发布状态：未发布。
- 已满足：项目范围、M2 目标架构与内容合同、引用/资产边界、实施拆分，以及“中国神话传说博物馆 + A 主 C 辅 Home 概念方向 + 中国阴间 + 钟馗 Entry”的工程参考与表现层替换边界已形成；未来商业化只保留隔离出口，不进入 M2 实现。
- 尚未满足：任笃行/张友鹤实际册页、《促织》跨见证材料、Painted Skin 的 Entry 证据消费关系与正文，以及四篇的翻译/资产/Collection 关系/状态和人工审校；M5-U4 的 Buttondown 账户级配置/真实 action/合成订阅联调、Tally 草稿发布与真实联调、U5 Plausible hook，以及真实 public origin、M6 public runner/Layout metadata/endpoint、最终 release-candidate QA、verification receipt、Vercel 项目与部署等后续门禁。CText/Giles Painted Skin 权利、`孽鬼/孽魅` 异文来源、真实键盘/200%/偏好、慢/阻断加载、图片失败、LCP/CLS、实际 fallback face、跨平台 fallback，以及 U3 与最终 inventory 的页面级人工/目标读者批准也尚未闭合。未验证项没有被改写成通过。本期未使用 ComfyUI，因此按 M3 合同不需要 workflow/model registry。
- Collection/Guide 资产、双语/CJK 门禁、M4 页面与人工判断已闭合各自范围；M5-U2 provider-neutral 合同/Fake 与 U3 inert review UI/Privacy 已落地。Buttondown 审核已通过只关闭等待审核门槛；U4 仍须分别授权并验证 Buttondown 账户级配置、真实 action 与合成订阅，以及 Tally 发布、精确 hosted link、合成数据写入、回查和删除。M6 内容方向、四篇研究、底本/单见证路线、证据最小物化与三篇纵切片已经建立；下一内容停点须另行选择并授权现有三篇编辑/双语审核、取得任/张实际册页，或 Painted Skin 逐篇写作。本批不自动授权图片、Collection/related 关系、真实联调、内容状态提升、Git 提交/push、public/Vercel、远端预览或发布；本次 Guide 本地提交来自 2026-09-05 后续独立授权，不包含 push。
- 是否可以关闭需求：否。

...
| 实施状态 | M1 已冻结；M2、M3、M4-U1 与 M4-U2 代码已完成 | U2 已建立显式 review 投影、共享页面壳、7 页 noindex 静态输出、Hero approved/current 接线与静态产物门禁；页面级视觉批准、U3-U5 与发布仍未完成 |
| 验证状态 | M1 工程/结构证据保留；M3、M4-U1 与 M4-U2 自动门禁通过 | 固定 Node 下 13 个测试文件/87 项测试、`astro check` 53 文件零诊断、7 页静态 build 与 14 个 v2 页面响应式输出通过；七个 local master 与 22 个 current 响应式目标回归通过。M4 浏览器、正式字体和人工视觉门禁仍未执行 |
| 发布状态 | 未发布 | 未创建托管项目、域名或发布配置 |

- 当前权威结论更新时间：2026-08-29。
- 当前提交基线为 `c606f5aab92d908ff2935c5b7212ad5066636a50`（`M3 completed`）；M2 历史基线为 `f258227da1b5a73f22c87ec99722243742db0ba0`。M4-U1 接管前，HEAD、`main` 与本地 `origin/main` 对齐且工作树干净；本次未执行 fetch，不能据此证明服务器端状态。
...
- 是否可以立即执行开发：否。M4-U2 代码与自动静态门禁已完成，当前应停止并等待 Project owner 页面级评审；U3-U5、dev/preview 服务、依赖、Git 写入、远端预览或发布没有获得授权。
...
- 下一项允许动作：停止并提交 M4-U2 三页候选供 Project owner 页面级评审；U3-U5、服务、依赖、Git 写入和发布须另行授权。
...
- 状态：M4-U1 已完成；M4-U2 页面源码与自动静态门禁已完成，等待 Project owner 页面级评审。U3-U5、SEO 发布输出和浏览器验收尚未开始且未获授权。详细合同见 [`003-pages-exploration-seo.md`](003-pages-exploration-seo.md)。
...
| 视觉家族 | 六个 Collection Hero、中性 Home、统一 Article、无图与灰度 | 同一现代品牌、不同主题展厅；Collection 只覆盖批准 token/资产/动效；文章阅读系统固定 | 中性 Home、中国阴间 Collection、钟馗 Entry 与 3×2 家族探针已作为工程参考冻结；无图/灰度和 realm token 静态/浏览器检查有证据。M3 钟馗五项 current 用途已通过资产级终审；Hero v1 手部缺陷被 v2 定向返修，旧版本保留审计。旧原型外观、真实页面与完整页面级视觉验收仍未批准，移交 M4 |
...
| 页面 | Home/Collection/Entry | 390/768/1440px 无阻塞布局问题；Home 中性、Collection 主题独立、Entry 阅读统一 | M4-U2 三页真实候选与最小全局入口已实现并通过静态语义/链接门禁；未启动服务或浏览器，三档响应式与 Project owner 视觉批准仍未执行 |
...
| 无障碍 | 键盘、focus、对比、语义、reduced motion | WCAG 2.2 AA 无阻塞问题；项目目标满足 | U2 静态代码包含 skip link、语义标题、manifest alt/caption/disclosure、2px focus、44px 目标和 reduced-motion 路径；未启动浏览器，真实键盘、视口、缩放与对比复核仍未验证 |
| 渐进增强 | 禁用 JavaScript/第三方失败 | 正文、来源和核心导航仍可用 | U2 构建产物没有客户端 JavaScript，原生 details 移动导航、Sources 和核心链接默认可见；禁用 JavaScript 的真实浏览器验收仍未执行 |
...
| 本地 Git 初始化与用户基线 | 当前项目根 `F:\codex-project\mythic-china` | 代理在初始化会话只执行 plain `git init`；用户随后建立 `main`、提交与 `origin` | 用户分别确认初始化与其自行 Git 操作 | M2 历史基线为 `f258227`；M3 已由用户提交为当前基线 `c606f5`。M4-U1 接管前 HEAD、`main` 与本地 `origin/main` 对齐且工作树干净，未 fetch 核验服务器端状态；代理未执行 add/commit/push |
...
| M4-U2 页面基础与首个真实纵切片 | 当前工作区已列明源码、测试、package scripts 与文档 | noindex review shell、7 页真实路由、Hero current resolver、测试和静态回归；不改内容/Schema/资产/依赖/lockfile | Project owner 明确要求开始 M4-U2 | 代码与自动静态门禁已完成，等待页面级评审；不含 U3-U5、服务、Git 写入、部署或发布 |
...
- 结果：M1 工程参考基线已冻结；M2 已由用户提交为 `f258227`；M3 完成并形成 `c606f5` 基线。M4-U1 合同与 M4-U2 noindex review 页面源码、测试和静态门禁现已完成，等待三页候选的 Project owner 页面级评审。
...
- 未完成/未执行：dev/preview 服务、浏览器自动化、390/768/1440 实际视口、键盘全链、reduced-motion、禁用 JavaScript、真实 200% 缩放、批准字体/慢加载/跨平台 fallback、实际字形命中、Project owner 页面级视觉批准、U3-U5、Git 写操作、部署和发布均未执行。
...
- 版本身份：当前提交基线仍为 `c606f5aab92d908ff2935c5b7212ad5066636a50`（`M3 completed`）；M2 历史基线为 `f258227da1b5a73f22c87ec99722243742db0ba0`。M4-U1/U2 改动尚未提交，代理未执行 `git add`、commit、push 或发布。

## 13. 当前最终结论

- 需求状态：草拟；M1 工程冻结决策、设计体系、A 主 C 辅的 Home 概念方向、首个 Collection、钟馗 Featured Entry、M2 技术/内容合同与 M4 第 1–9 项详细合同已确认；后置事项按对应单元再确认。
- 实施状态：M1 已完成并冻结为工程参考基线；M2 已完成并由用户提交；M3-U1 至 U5、M4-U1 与 M4-U2 代码/静态门禁已完成；U3-U5 未开始。
- 验证状态：M3 完整工程门禁与 22 个响应式输出复核通过；M4-U2 的 13 个测试文件/87 项测试、Astro 53 文件零诊断、7 页 noindex 静态输出、14 个 v2 页面图片和内部链接门禁通过。真实浏览器、正式字体与人工视觉仍未验证，三页尚未获批准。
- 发布状态：未发布。
- 已满足：项目范围、M2 目标架构与内容合同、引用/资产边界、实施拆分，以及“中国神话传说博物馆 + A 主 C 辅 Home 概念方向 + 中国阴间 + 钟馗 Entry”的工程参考与表现层替换边界已形成；未来商业化只保留隔离出口，不进入 M2 实现。
- 尚未满足：首个真实 Home、Collection 与 Entry 通过 `DESIGN.md` 第 12 节完整浏览器验收、正式字体和页面级人工视觉批准；公开品牌/域名、托管、第二个 Collection 等后续门禁。本期未使用 ComfyUI，因此按 M3 合同不需要 workflow/model registry。
- 下一项允许动作：停止并提交 M4-U2 页面候选供 Project owner 评审；没有自动延续至 U3-U5。
- 是否可以关闭需求：否。

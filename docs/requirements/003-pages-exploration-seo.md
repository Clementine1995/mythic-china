...
- 固定 M4 的页面职责、公开 URL、内容状态到路由/索引的投影、public/SEO 纯基础设施、首个真实纵切片、实施单元和本地验收门禁，并记录 public artifact 与最终 release QA 向 M6 的移交。
...
- 把 M1 原型批准、M3 资产批准、M4 本地页面批准、M6 public artifact 验收或最终发布混为同一结论。
...
- 是否可以进入下一工程单元：可以。治理、机器门禁加固、Collection/Guide Hero、双语术语、Source 标题 locale、CJK 静态生产链、Windows/local 部分预检、功能页修正、U5A 字体样张及 M4 当时 8 页 Project owner 视觉判断已闭合各自范围。M5-U2 provider-neutral 合同/Fake 与 U3 inert review UI/Privacy 已完成；Project owner 于 2026-09-04 确认 Buttondown 账户审核已通过，Tally Free 草稿仍未发布，U4 仍未完成。后续内容批已使四个 draft Entry 均形成证据消费关系与正文、通过人工双语审核，并接通两个三篇 Collection 路径；012 又为四篇 Entry 与 Liaozhai Collection 完成五组 approved/current Hero 资产、生产记录和内容绑定。当前 review 精确清单为 14 页；VB6 已闭合四篇分类、两条 Related 编辑环、Liaozhai Featured（Painted Skin）与 6+2 ready。下一步仍须按具体轨道另行授权：外部交互处理 Buttondown/Tally 真实配置与合成数据；owner 已批准首发 6+2 published，六篇目标日期 2026-09-10；内容轨道下一停点是独立的 origin/public assembly。M6 public artifact assembly、键盘/缩放/偏好、慢加载/故障、LCP/CLS、实际 fallback 与跨平台 release QA，以及后续 Git 写入、Vercel 项目操作、部署或发布，均未被本批授权。目标读者 R2a/R2b 按 011 在 Public Beta 上线后执行。
...
- M4 收口时保留的发布风险：真实键盘/缩放/媒体偏好、慢或阻断字体、图片失败、受控 LCP/CLS、实际 fallback 可读性与支持平台回归仍未验证；当时 8 页正式英/CJK 字体、页面判断、最终三档基础矩阵、CJK 静态链与 U5A 自动门禁只关闭本地实现范围。最终目标读者检查按 011 移到 Public Beta live artifact。
- 本地 indexable SEO 已在 013 接线并检查；剩余发布门槛为完整候选准备、clean source 与针对最终 artifact 的 release QA。Collection 专属 Hero 与既有模板消费链已闭合；当前采用 text-only OG，因此用途匹配 OG 图片不是阻塞项。
- 推荐工程顺序：M5 外部交互详细需求与实施 → M6 完成 6 篇 Entry、至少 2 个 Collection、全部资产及 Project owner 的 `published` 决定 → 确认真实 HTTPS origin 并完成 public artifact assembly 实现 → Project owner 单独授权 clean commit → 从该 revision 重新构建、运行 public output verifier 与键盘/缩放/偏好、故障、本机字体、视觉及本地性能 QA → 生成列明未验证平台的 clean-source verification receipt → 经独立授权进入受保护预览，跨平台实机显示/fallback 及三地区性能按 011 在 Public Beta 上线后、结束 Beta 前补齐 → M7 生产发布可索引 Public Beta 并完成 live smoke/RUM 基线 → live R2a → R2b → 处置并由 Project owner 关闭正式 MVP 验证。dirty source 只能产生 nondeployable 诊断记录，提交后不得复用。没有自有域名时，可在 M6 单独授权建立 Vercel 项目身份以确认稳定 production alias/hostname，但不得部署 review 输出。public-release 的一个 Entry + 一个 Collection 只是纯技术 fail-closed 下限；每一步均须 Project owner 分别授权。

### 1.1 已确认事实

- M4 收口时的 review 构建精确生成 Home、Explore、Collections、About、一个 Collection、两个 Entry 与 direct-only type specimen，共 8 个 `noindex, nofollow` 页面；DebugLayout 已删除，样张不进入全站导航、review candidate 或 release/public 投影。M5-U3 后新增 Privacy，形成 9 页历史基线；2026-09-04 的四个 draft Entry 通过既有动态路由生成 direct-only review 页面，009 后又新增 Liaozhai Collection 页面，当前全局 review 构建为 14 页。Footer Newsletter 与六个 Entry 的 Reader Request 均为 inert review 入口，不改变 M4 release/public 投影。
- 当前动态路由按显式 review 投影生成全部 non-archived 记录；独立纯 release 投影只选择 published，并为 Explore/Collections/Related Entries 提供确定性 view model。固定 Home 纵切片与由同一固定 Collection 及其 `entryIds` 精确派生的 review-only 索引候选架，是仅有的 non-published 选择例外；候选架不消费全部 non-archived inventory。M4-U4A 新增 public 最小数量与 SEO/artifact 纯函数，013 现新增显式 public intent；其动态路由只消费 published 投影，review 仍使用原有候选边界。
- VB6 ready 审核后，owner 独立批准六篇 Entry / 两个 Collection 均为 published，六篇目标日期统一为 2026-09-10；四篇分类、两条 Related 编辑环和 Liaozhai Featured（Painted Skin）已闭合，阴间 Featured 保持 Zhong Kui。Underworld Guide 复用已有 CMA 来源补末段近引，书目为四条；正文含义、其余证据、Hero 与事实核查日期保持，earliest-known 仍为空。Related 继续只展示 published 目标，因此当前六篇 review 页各显示一个既定目标；合集正反路径与 Featured 可供本地评审。
...
- 中国阴间 Collection 的 `entryIds` 依次包含 non-published `chinese-underworld-guide`、`ten-kings` 与 `zhong-kui`；Liaozhai Collection 依次包含 `liaozhai-reading-guide`、`painted-skin` 与 `fighting-cricket`。既有内容图禁止 published Collection 引用未发布 Entry，因此不能在页面层过滤尚未发布的成员来制造部分公开合集。
- 仓库已有共享页面壳、Home、Explore index、Collections index、保持四节结构的 About 与 direct-only type specimen；About 现可见说明 `Mythic China Editorial` 团队身份。4 份英文 WOFF2 与 Source Han Sans 2.005R 派生的 SC/TC × 400/500/600 六份静态 WOFF2 已自托管接线并受静态门禁约束，样张不修改其 inventory；默认 `dist` 仍没有 canonical、Sitemap、RSS 或结构化数据；013 在真实 origin 下将这些输出生成至 `.local/public-build/`，与 review 隔离。
- M3 资产通过本身不等于 M4 页面批准；Home/Entry 的 Hero v2 页面使用、裁切、排版与响应式已随 M4-U2 页面方向确认。Project owner 又阶段性接受当前内容/模板，并于 2026-08-31 分别批准 Collection desktop 02/mobile 01 与 Guide desktop A2/独立 mobile 的组合、publication rights、五审、exact-canvas、公开文案与正式资产链。静态 Collection 与 Guide 页面已各自消费自己的 Hero；2026-09-02 当时 8 页的正式字体与页面观感也获明确通过。尚未批准的是 M6 最终 public artifact 的完整视觉/平台 QA 与发布资格；目标读者研究在 M7 Public Beta 上线后执行，两者均不改变 M4 本地实现完成事实。
...
- Explore/Collections 的本地评审页只在对应 published 列表为空时展示一个明确标注 `Not published` 的候选架；有 published 列表时不调用候选 helper、不展示候选架或其说明。该候选架只能复用固定 Home Collection 与其 `entryIds`，不得把其他 non-archived 内容自动暴露为全局候选。
...
- 精确 canonical origin 已确认并在 013 显式配置为 `https://mythic-china-beta.vercel.app`；空项目与 Production hostname 来自用户网页操作和截图，尚无部署。任何 placeholder、generated/branch URL 或静默 fallback 都不能成为生产 canonical。
...
- 英文正式字体已有 WOFF2、hash、OFL、alias/token 与 preload 候选；CJK 已固定上游 commit/input hash、Hans/Hant 字符集、六份静态 WOFF2、许可证/RFN/FONTLOG、精确 `unicode-range` 与 cmap/HTML 语言门禁。当前浏览器页面效果已获 Project owner 通过；慢/阻断加载、实际 fallback 可读性与支持平台证据仍待 M6。
...
- 原有两篇 Entry 已形成完整候选叙事与来源层级，四个新增 Entry 也已建立各自证据消费关系、正文，通过人工双语审核，并完成 approved/current Hero 资产与内容绑定；Liaozhai Collection 同步取得独立 Hero。六份 Terminology 均为 `bilingual-approved`，Painted Skin 短译 Claim 为 `verified`。四条馆藏标题 locale 与 CJK 静态生产链已闭合，钟馗 earliest-known 保持空。VB6 已闭合四篇分类/Related、Liaozhai Featured 与 6+2 ready；目标读者反馈为 0，并按 011 延后到 Public Beta。M6 的逐项 published 决定与六篇目标日期 2026-09-10 已确认；public artifact、字体/故障/跨平台 release QA 与发布资格仍未闭合，因此不等于发布就绪。
- 四条候选页的 390/768/1440 正式页面回归已经完成；2026-09-02 又对最终 U5A direct-only 样张和其余 review 页面运行 8 页 × 三档共 24 个实际组合，最终生产 pinyin、冻结混排行与显式 CSS 映射现有浏览器基础证据。最终样张覆盖英文 display/story/body、SC/TC 400/500/600、困难字形与 fallback-only probe，并在三档观测到 10 个字体资源；Project owner 随后明确通过全部 8 个页面。当前仍不能可靠证明真实键盘全链、200%、禁用 JavaScript、启用 reduced motion、慢/阻断字体、图片失败、LCP/CLS、实际 fallback face 和跨平台字体。
...
M2/M3 已把内容、证据和版本化视觉资产建成可靠的静态输入；M4-U2 已用 noindex review 页面替换 debug templates，并完成 system fallback 字体下的页面方向评审。004 又形成英文正式字体、CJK 静态生产链以及 Chinese Underworld Collection/Guide Hero 的 approved/current 生产链与静态接线，U5A 再补入 direct-only 字体样张与当前可控浏览器证据，M4 本地实现因而已经闭合。但当前仍不是 deployable/public 输出：真实 origin、public 页面/output 接线、完整内容 inventory 与最终 artifact 的字体/故障/跨平台 release QA 必须在 M6 独立关闭，不能把 `editorial-review`、单项资产批准或 noindex 构建当作发布资格。
...
- 共享生产 shell、导航、Footer、响应式图片接线和无 JavaScript 基础阅读；页面 metadata 的纯构建合同由 U4A 提供，实际 public 接线留 M6。
- canonical、Open Graph、Sitemap、RSS 与约定 JSON-LD 的纯 builder、状态/身份/序列化正反门禁；实际 `/sitemap.xml`、`/rss.xml` 与页面输出留 M6。
- 首个真实纵切片的本地响应式、字体静态链、资源/语义与人工视觉复核；发布环境无障碍、故障、跨平台和性能矩阵留 M6/M7。
...
- M6 的其余首发内容、第二个 Collection、批量插画、完整 inventory 的状态决定、public artifact assembly、最终 release-candidate QA 与受保护预览；内容 `ready` 只由内容拥有的门禁决定，不要求也不得为了页面预检提前提升为 `published`。
...
Markdown / YAML + visual records
  ...
  -> M5-U3 nine-page noindex baseline with inert Newsletter / Reader Request surfaces + Privacy route
  -> four direct-only draft Entry routes from the existing review projection
  -> thirteen noindex review pages + forty-two emitted Hero images + ten hash-locked WOFF2
```
...
- Public 构建至少要求一个 published Entry 与一个 published Collection；固定导航任一核心索引为空时构建失败，不生成可部署的空 Explore/Collections。该数量只是 M6 public artifact assembly 的纯技术 fail-closed 下限，不是 MVP 预览或生产发布资格；M6/M7 仍分别执行 6 篇 Entry、至少 2 个 Collection 和生产发布/发布后只读基线验收。
...
- M4-U2 当时的钟馗候选缺少多数正文输入，因此 U2 只能渲染真实存在的字段并省略不存在的内容区块，不得从 Claim statement 自动拼成文章或用 lorem/AI 草稿填充。后续两篇 Entry 形成 `editorial-review` 正文候选；2026-09-04 新增的四篇在该时点仍为 draft owner，其中 Ten Kings、Fighting Cricket 与 Liaozhai 导读已有证据受限首稿，Painted Skin 仍为空。此处保留当时快照；四篇后续均已形成草稿并通过人工双语审核，当前状态以第 13 节和 001 为准。
...
- Home 实际使用 approved/current Hero v2 已在 M4-U2 浏览器评审中获得页面方向确认；该确认不把 Zhong Kui 固化为长期全站身份，也不替代后续正式字体、M4 当前页面判断或 M6 最终 public artifact/发布批准。前两项后来已随 M4 闭合；M6 release-candidate QA 与 M7 生产责任仍保持未完成。
...
2026-09-07 无账号本地子单元补充：M5 已有 `/privacy/` review 页面，但旧 public 纯 builder 未覆盖它。本批将 `privacy` 精确映射到 `/privacy/`，结构化数据采用普通 `WebPage`，不使用 `AboutPage`、`Article` 或 `CollectionPage`；这属于本批实现选择。固定 Sitemap 静态清单扩为 `/`、`/explore/`、`/collections/`、`/about/`、`/privacy/`，按此顺序各一次，缺失/重复/其他路径继续失败；Privacy 不进入 RSS。不新增 URL、内容字段、origin 配置或页面输出，复用现有 fixture 仅做纯函数验证。该子单元已完成并通过定向与完整本地检查；真实 Privacy 文案仍须在 M6 根据实际 hosting/origin/provider 状态复核，review 继续 noindex 且不生成发布型 metadata/XML，public assembly 与发布门槛不变。
...
### M4-U4A SEO 纯基础设施与 M6 public artifact assembly

- 目标：M4 冻结 canonical、Open Graph、Sitemap、RSS 与约定 JSON-LD 的纯构建合同；M6 再把它们接入真实 public artifact。
- 责任迁移：原计划 M4-U4B 不再定义 M4 完成，改由 M6 public artifact assembly 承接。进入条件为真实 HTTPS site origin、author/publisher 身份、完整 6 篇 Entry / 至少 2 个 Collection published inventory、正式 loader/validator/resolver、已批准字体/CJK 静态链与用途匹配的页面 metadata；Project owner 单独授权配置/文件范围。pending 发布 QA、system fallback、早期 M4 页面证据或 dirty-worktree `dist/` 不能替代可部署证据。若需新依赖，再单独授权依赖。M4-U4A 只冻结配置名、校验与纯 builder，因此可以在没有真实 origin/发布内容时完成且不制造假 public artifact。
- 交付：统一 metadata builder、published-only artifacts、绝对 URL 和 build-time fail-closed 测试。
- 不交付：域名注册、DNS、托管、部署、分析或搜索。
- 完成条件：M4-U4A 的状态矩阵正反测试、canonical/OG 一致性、Sitemap/RSS 内容与排序、JSON-LD 可解析且无 placeholder identity 已通过。M6 public runner 只能从正式 loader 经 Schema、content graph、visual graph/file inventory 与 approved/current resolver 后进入 published projection；不得 raw-load 记录后直接调用 pure builder。独立 public verifier 拒绝 review 页面、未发布路由、远端子资源、未经批准的客户端 JavaScript 和缺失 metadata/XML。
- M6 编码停止点：真实 origin、published inventory、正式 loader 链和字体/CJK 静态 readiness producer 必须先冻结；在此之前不得开放 public intent。本地浏览器 QA 尚为 pending 时，runner 可以生成明确标记 nondeployable 的 QA artifact，以便执行后续检查，但不得生成最终 receipt 或进入预览。当前明确的时点调整为 [011 第 2.2.1 节](011-public-beta-validation.md#221-跨平台实机验收时点)：macOS/iOS/Android 实机显示与字体 fallback、美国东/西部和欧洲性能在 Public Beta 上线后、结束 Beta 前补齐；其他本地项通过后以明确列出未验证平台的 receipt 进入单独授权的预览。解析 Markdown、单独修改 `font-assets.json.status` 或依赖人工记忆都不能替代完整 evidence。
- 来源证明：M6 assembly 实现稳定后，须由 Project owner 单独授权形成 clean committed source；从该 revision 重新构建 artifact、运行 verifier 与预览前本地 QA 后，才能生成绑定 clean source revision、lock/source digest、public intent、verifier/QA 结果及 `dirty: false` 的预览 verification receipt，并明列跨平台实机未验证项；实机显示与 fallback 按 011 最新决定在 Public Beta 上线后、结束 Beta 前完成。dirty source 只能产生 nondeployable 诊断记录，提交后必须重建、重验并重发 receipt；verifier 只证明输出语义，不冒充部署来源证明。远端预览再把该 clean committed source、验证结果与具体 deployment target 绑定为 `validated_source_identity`；若允许复用不可变制品，receipt 还必须包含完整 artifact inventory/digest。当前 `font-assets.json.status` 只是被测试锁定的状态标签；M6 runner 必须把 pending 传播为 nondeployable QA artifact，预览门禁显式消费发布字体 QA 状态或等价的可追溯 evidence producer；仅 011 第 2.2.1 节明确后移的实机及三地区性能 pending 可保留至结束 Beta 前，其他 pending 或失败仍拒绝相应预览或生产放行，不能靠人工记忆或只改 status 冒充通过。
...
- M6 public artifact assembly 当前状态：013 已获本地实施授权并完成接线，正式 origin、published 路由、metadata、Sitemap/RSS/robots 和独立 public output verifier 已存在。输出基于未提交源，只作本地诊断；完整发布候选、clean source identity、最终 QA/receipt 仍未完成。public 只允许显式 intent 与已确认 origin，不创建 readiness adapter 或放宽部署授权。

### M4-U5 本地实现验收与 M6 release-candidate QA

- M4 目标：用 noindex review 页面验证首个真实纵切片、共享表现层、正式字体静态链、三档基础布局、资源/链接、Hero art direction 与 Project owner 当前页面判断，不把本地证据冒充发布资格。
- M4 完成事实：M4-U2/U3/U4A、Collection/Guide Hero、双语术语、Source 标题 locale、CJK 静态生产链、U5A 样张、最终 8 页 × 三档 24 个组合、四条 Hero 页 12 个组合及当前 8 页人工视觉判断已闭合。真实键盘、200%、偏好/故障、性能、实际 fallback 与跨平台未验证，但不再定义 M4 完成。
- M6 release-candidate QA 进入条件：M5 外部交互边界已冻结；M6 已完成 6 篇 Entry、至少 2 个 Collection、全部资产与 Project owner 的 `published` 决定，已用真实 origin 完成 public artifact assembly 实现；Project owner 已单独授权形成 clean committed source，且最终 QA artifact 已从该 revision 重新构建。
- M6 QA 交付：在该 clean revision 重新构建的同一最终 Home/Explore/Collections、全部受影响动态页、M5 交互、metadata/XML、最终字体/资产/CSS 与真实关系上执行自动 output verifier、390/768/1440、真实键盘/focus、JavaScript-disabled、reduced motion、200%、字体/图片故障、支持平台 fallback、链接、本地性能、DESIGN 第 12 节适用的上线前项目及 Project owner 最终视觉确认。其中跨平台实机按 011 第 2.2.1 节在 Public Beta 上线后、结束 Beta 前完成，其他本地项在进入预览前通过；没有把未测项目记为通过。COMPETITIVE_AUDIT 第 6.3 节的目标读者问题随 010/011 移到 M7 Public Beta。
- 失效规则：实质内容、状态、模板、CSS、字体、Hero、manifest、origin、public metadata、runner、XML 或交互变化时，只按影响范围重跑对应 QA；失败阻塞 receipt、远端预览或发布，不把 M4 历史实现重新标为未完成。
- 当前状态：M4 本地实现验收已完成；M6 public artifact assembly、release-candidate QA、receipt、远端预览与 M7 生产均未开始、未授权。后续 VB6 已独立审核 6+2 到 ready，owner 随后批准全部 published，六篇目标日期为 2026-09-10；M4 完成及证据物化本身不自动提升状态。

钟馗完整研究、正文、术语/译文和状态提升不自动塞入既有 M4-U2 至 M4-U4A。M6 承接其余首发内容、第二个 Collection、批量资产、完整 inventory 的人工 `published` 决定、public artifact assembly 与最终 release QA；未完成时 public intent 和远端预览保持 fail closed。
...
| 页面 | 六类内容/功能 URL、direct-only 样张、语义顺序、静态导航、review preview、无 JS | 路由/链接完整；release 空状态与 review 候选架分离；样张不进入导航/public；About 四节可见；键盘语义不依赖脚本 | 聚合 `pnpm run check` | 通过：原 7 页加样张共 8 页精确 inventory、全部内部链接、5 份完整 Source、Explore 两个 Entry 候选、Collections 一个 Collection 候选、其他五页无 preview 标记、样张无全站入口、真实 release 空状态、About 四节/身份锚点、skip/main、原生 details 导航与零客户端 JS 均通过；真实键盘操作未执行 |
...
| 字体静态链 | 字符集、上游输入、派生输出、CSS、cmap、HTML 语言边界与样张 | 4 份英文与 6 份 CJK WOFF2 全部 hash-locked；SC/TC cmap 与批准集合精确相等；Han 不继承 `en` 或 generic `zh`；样张不放宽原 7 页字符合同 | typography Vitest + review output verifier | 通过：SC 65 个、TC 36 个 required 码位，六份静态 CJK WOFF2、OFL/RFN/FONTLOG、精确 `unicode-range`、无 CJK preload、源码及 `dist` cmap/内部命名/weight/无变量轴和原 7 页实际 HTML 字符集均受门禁；样张另锁定 20 个 role/weight/style/lang/fallback 组合。实际 fallback face、慢/阻断与支持平台命中留 M6 |
| SEO | canonical、OG、Sitemap、RSS、JSON-LD | published-only、绝对身份一致、无 placeholder | M4-U4A 定向测试 + M6 public output 检查 | M4-U4A 纯模块通过：origin/身份、public 数量、kind/path、日期、metadata 唯一性、published ItemList、JSON-LD/XML 转义、Sitemap/RSS 排序正反 fixture；M6 页面/output 接线未执行 |
| 浏览器/无障碍 | 390/768/1440、键盘、200%、reduced motion、字体 | DESIGN 与 WCAG 门禁闭合 | M4 本地基础矩阵 + M6 最终 artifact QA | M4 范围通过：2026-09-02 最终 8 页 × 三档 24 个组合全部无横向溢出、破图、样张错配、card 重叠或 console 问题，20/20 sample 可见、10 个字体资源被观测，最终 pinyin/mixed DOM 与显式 normal CSS 的 computed style 正确；四条 Hero 页 12 个组合也选择正确 composition，Project owner 通过全部 8 个页面。真实键盘、200%、reduced motion、禁用 JS、慢/阻断字体、图片失败、LCP/CLS、实际 fallback face 与支持平台仍未验证，归 M6 release-candidate gate |
| 人工视觉 | Home A/C、Collection 主题、Entry 阅读器与功能页 | Project owner 明确确认页面，不继承 M1/M3 批准 | M4 当前页面评审 + M6 最终 artifact 评审 + M7 Public Beta 目标读者研究 | M4-U2 页面方向已确认；Project owner 本轮确认总体观感、完成三个功能页反馈复看，并于 2026-09-02 明确通过当前全部 8 个页面。当前 noindex 页的样张、Hero 裁切与阅读观感已闭合；未来完整 inventory 与 M5 交互留 M6，正式目标读者视觉在 M7 live artifact 上研究，M4 完成不提升 `ready` |
...
| M4-U5A noindex 字体样张与可控验证入口 | direct-only review route、作用域 CSS、独立策略/正反测试、既有 verifier、证据文档与一次 preview/browser | Project owner 明确要求先更新合同、再实现并运行当前工具可控场景，随后同意按整体审查修复 fail-closed 缺口 | 实现与自动门禁已通过 23 文件/279 测试、Astro 73 文件零诊断、8 页/42 图/10 字体/0 XML/0 JS；审查加固前候选完成过样张三档，PID `9628` 已停止。最终 DOM/CSS 的后续证据见下一行；本行保留历史范围 |
| M4-U5 最终三档与人工验收交接 | 同一未提交源的完整门禁、一次 local preview、8 页三档/四页 Hero/交互/字体证据、直接修复与稳定文档 | Project owner 于 2026-09-02 单独授权最终 U5 浏览器与人工证据批次 | 24 个实际视口组合、12 个 Hero 组合、20/20 sample、10 个观测字体资源及 clean console 已通过；未发现代码缺陷。Project owner 明确通过全部 8 个页面；验收标签已清理，PID `31960` 已停止且 4321 无监听。真实键盘/200%/偏好/故障/性能/实际 fallback face/跨平台仍未验证 |
...
- 剩余风险（U3 历史时点）：当时真实内容没有 published 对象，因此单项/多项 release 行为只由纯 fixture 证明；静态语义门禁不证明真实键盘交互。当时 M4-U4 仍被 HTTPS origin 与 author/publisher 身份阻塞，随后身份、Collection Hero/模板与 M4 本地页面判断均已闭合。2026-09-02 的现行责任迁移见 12.14：真实 origin、published inventory、public assembly 与最终字体/浏览器/性能/视觉门禁现归 M6。
...
### 12.11 M4-U5A：noindex 字体样张与可控验证入口

- 截至该 2026-09-01 单元：M4-U5A 的最终实现与自动门禁已完成；Windows/local 浏览器证据覆盖审查加固前候选，最终 pinyin/mixed DOM 与 CSS 当时仍待重新授权复跑。入口存在和本节自动层通过不等于 Project owner 视觉批准，也不关闭完整 M4-U5；后续最终三档结果见 12.13。
- 路由与 inventory：新增仅允许显式 `review` intent 生成的直达页 `/review/type-specimen/`；默认 review 输出由原 7 页精确增加为 8 个 HTML。该页不得进入 Header/Footer、Home、Explore/Collections review candidate、release/public projection 或六类 public 页面合同，未来 public intent 必须不生成该路由。
- 样张合同：英文 display 以现有 400、560、600、650 代表性权重覆盖 hero/UI/metadata/pinyin；story/body 只覆盖现有 400 normal、600 normal 与 400 italic face。SC/TC 分别以 `lang="zh-Hans"` / `lang="zh-Hant"` 覆盖 400/500/600、冻结 required 字符、拼音与标点；`测` / `測` 只在对应 locale、同一 CJK family stack 的 fallback-only 节点中出现，不得加入 required、`unicode-range` 或自托管 cmap。
- 自动门禁：以 [`cjk-character-sets.json`](../../src/assets/fonts/cjk-character-sets.json) 和 [`DESIGN.md` 5.2.1](../DESIGN.md#521-字体验收样张) 为唯一字符事实源；原 7 内容页继续精确匹配真实 content set，样张独立匹配 required 与 fallback-only set。缺失/额外页面、字符、locale、role、weight、style、fallback 标记或重复组合必须失败；输出仍精确为 42 个 Hero 图片、10 个 hash-locked WOFF2、零 XML、零客户端 JavaScript，并逐页保持 `noindex, nofollow`、零 canonical/Open Graph/JSON-LD。
- public 负边界：纯 release/SEO/artifact fixture 必须显式证明 `/review/type-specimen/` 不属于 public static route、canonical、Sitemap、RSS 或 JSON-LD 投影；当前没有 public runner，因此只能记录纯 builder 与 review 输出的负证据，不能冒充真实 public artifact 验证。
- 自动验证：固定 Node/Corepack 下完整 `pnpm run check` 通过 23 个测试文件/271 项测试、Astro check 73 文件零诊断和精确 8 页/42 图/10 字体/零 XML/零客户端 JavaScript output verifier。正反 oracle 基于 HTML5 产物树和逐文件闭合的实际应用 stylesheet/inline CSS 拒绝 review inventory、active markup/stylesheet/font 后缀、导航、字符、生产 pinyin `lang`、DESIGN 冻结混排行/inline Hans boundary、role/content role、DOM containment/标签关联、family/weight/style/synthesis、fallback、robots head/bot override/hreflang/SEO/JS、import/条件/旁路样式与等价 direct-link 漂移；release artifact 与 SEO fixture 显式拒绝样张路径。原 7 页继续独立匹配真实 CJK content set。
- 可控浏览器历史结果：固定 preview 返回 PID `9628`，实际监听 `[::1]:4321`，浏览器使用 `http://localhost:4321/review/type-specimen/`。审查加固前候选在 1440×900、768×900、390×844 均无横向溢出，20/20 样本有布局盒，`document.fonts` 精确为 10 / `loaded`，当时声明的 display/story/body 与 SC/TC 400/500/600 计算 family/weight/style/lang 一致，页面为唯一 `noindex, nofollow`、零 canonical/OG/JSON-LD、零图片/脚本，console 无 warning/error；Guide 内容页 Hero 正常完成加载且 natural size 非零。其后最终实现修正 pinyin `lang`、补入冻结混排行与显式 normal CSS，因此该矩阵不证明最终 DOM/CSS；最终三档需重新授权。fallback 节点当时沿同一 CJK stack 可见，静态 cmap 证明 `测`/`測` 不在自托管字体，但控制面不能读取实际渲染 fallback face，故不声称具体系统字体命中。
- 该 2026-09-01 单元的未验证场景：最终审查加固样张的 390/768/1440 浏览器复跑当时尚未重新授权；in-app Browser 只提供 viewport/visibility，没有网络拦截/节流、JavaScript 开关、media preference 或性能面板；即使临时显示浏览器，控制面也没有实际分发 Tab/Enter/Space 默认动作，Ctrl+加号后 DPR、visual viewport scale 与 inner width 均未变化。因此真实键盘全链、200% zoom、JavaScript-disabled、启用 reduced motion、慢/阻断字体、图片失败、LCP/CLS 与 macOS/iOS/Android fallback 均保持未验证，不把工具限制误报为页面失败或通过。
- 环境收口：临时 viewport 已 reset，两个测试 tab 已关闭；固定 Node 执行 `astro preview stop` 后 PID `9628` 停止，`netstat` 回查 4321 无监听。本批未运行非默认 `visual:build:check`，因为未改视觉 master/manifest/buildPlan；其 11 个 master/50 个响应式输出证据继续沿用。
- 不做范围：不改内容状态、Source/Claim/Terminology/Hero、manifest/production record、字体二进制/子集/字符集/RFN/FONTLOG、依赖、public intent、M4-U4B、M5/M6、Vercel、部署或发布；`font-assets.json` 的 `browser-review-pending` 不因 U5A 基础设施存在而提升。

### 12.12 M4-U5A verifier fail-closed 加固

- 结果：按 Project owner 对整体审查结论的确认，既有 U5A 自动层已关闭五类可复现旁路：功能伪类 selector list、CSS 几何/非渲染隐藏、源文本伪造语义壳、未落入输出 inventory 的 HTML/CSS 子资源，以及 `dist` symlink/junction/未知条目。该结果只提升自动门禁可信度，不生成新的浏览器或视觉证据。
- 实现边界：parse5 活跃树核对根语言、skip/main 与移动 disclosure 真实父子关系；资源扫描继续进入安全相关 surface，并把 stylesheet、font/image `src/srcset/imagesrcset`、poster、SVG image/feImage、inline/style CSS `url()` 闭合到 emitted URL。样张只允许 CSS/WOFF2 资源，并拒绝媒体节点、脱离精确 grid 的 card 与 transform/clip/offscreen/zero-size 隐藏。输出树逐项 `lstat`，任何链接或未知类型失败。
- 验证：旧实现上的新增负例先出现 8 项失败；修复后固定 Node/Corepack 下完整 `pnpm run check` 通过 23 个测试文件/279 项测试、Astro 73 文件零诊断、8 页/42 图/10 字体/零 XML/零客户端 JavaScript verifier。真实临时 junction 回归通过；构建后压缩的 `:before` 等价形态经精确既有装饰允许项复核后通过。
- 截至该加固单元的保留边界：本批未启动服务/浏览器，未改页面、内容、字体/图片资产、依赖或配置，未运行非默认视觉生产门禁，也未执行 Git、Vercel、部署或发布动作。12.11 的候选三档只是历史观察；最终样张三档当时尚未闭合，后续结果见 12.13。真实键盘/200%、偏好/故障、性能、实际 fallback face、跨平台与 Project owner 视觉判断继续未闭合。

### 12.13 M4-U5 最终三档与人工验收交接

- 授权与源：2026-09-02，Project owner 单独授权最终 U5 浏览器与人工证据批次；范围为同一未提交 source/worktree 的完整门禁、一次 local noindex preview、最终三档/字体/Hero/交互证据、直接修复、Project owner 判断面与稳定文档，不含状态提升、依赖、public/U4B/M5/M6、Git、Vercel、部署或发布。HEAD/branch 为 `e2893d14d4960f71fe75bd240971aaa88656511c` / `main`，工作树承接 U5A 与 verifier 加固；固定 Node 24.16.0、Corepack 0.35.0、pnpm 11.22.0 的完整 `pnpm run check` 再次通过 23 文件/279 测试、Astro 73 文件零诊断与 8 页/42 图/10 字体/0 XML/0 JS verifier。
- 浏览器结果：PID `31960` 在 `[::1]:4321` 提供 `http://localhost:4321/`。三个独立 Codex In-app Browser 标签的实际尺寸为 390×844、768×900、1440×900；最终 8 页 × 三档共 24 个匹配组合全部无横向溢出、破图、样张 computed-style 错配、card 重叠或 console warning/error，保持唯一 main/H1、`noindex, nofollow`、零脚本、font status loaded 与正确导航断点。样张三档均为 20/20 sample、14 张非零 card、无隐藏/裁切，资产 inventory 观测到 10 个 WOFF2、0 image/0 script；最终 pinyin、mixed/inline CJK、SC/TC 400/500/600、story/body/italic 的 computed family/weight/style/synthesis/`lang` 均与合同一致。
- Hero 与交互：Home、Collection、Guide、Zhong Kui × 三档共 12 个组合均在 390px 选择 mobile composition、在 768/1440px 选择 desktop composition；图片 complete/natural size 非零、informative alt 与 caption 可见，Collection copy/caption 不相交。鼠标移动菜单与样张 checklist 可开合，四个移动菜单项和 checklist 返回链接均为 44px，关闭后焦点留在 summary；skip、mobile summary 与 specimen summary 经 locator 聚焦时显示 2px outline。没有发现需要修改业务代码的缺陷。
- 能力边界：实际重试中，locator/CUA 的 Tab/Enter/Space 未触发浏览器原生默认动作，Ctrl-plus 前后 DPR、visual viewport scale 与 inner width 不变；当前求值作用域没有可靠 Performance API，Browser 也没有 JavaScript-disable、media emulation、网络/字体/图片阻断或跨平台能力。截图接口在背景/显示和原/新标签上均未能取图，故仓库不保存替代截图。Project owner 通过全部 8 个页面的人工判断不改变这些工具边界；真实键盘、200% zoom、JavaScript-disabled、reduced motion、慢/阻断字体、图片失败、受控 LCP/CLS、实际 fallback face 与 macOS/iOS/Android fallback 继续明确未验证，不把工具限制记成页面失败或通过。
- 人工结论与环境收口：Project owner 检查全部 8 个路由后明确回复“这些页面通过”，闭合当前样张、Hero 裁切与页面阅读观感。浏览器清理确认验收标签已关闭并重置临时 viewport；固定 Node 停止 PID `31960` 后，进程不存在且 4321 无监听。当前内容仍为 0 published / `editorial-review`，`font-assets.json` 仍为 `browser-review-pending`。本批未改页面、内容、字体/图片资产、依赖或配置，非默认 `visual:build:check` 不适用；截至本记录未执行 Git 写入、Vercel、部署或发布。

### 12.14 2026-09-02 M4 范围重定界决定
...
### 12.15 2026-09-04 四篇证据最小物化对 review 输出的影响

- 四个新 Entry 仅建立空 `draft` owner；既有 `[slug].astro` review 投影因此自动生成 `/explore/ten-kings/`、`/explore/liaozhai-reading-guide/`、`/explore/painted-skin/` 与 `/explore/fighting-cricket/`。没有新增或修改模板、导航、CSS、Hero、字体或 public builder。
- `font-specimen-policy.mjs` 与 `verify-m4-u2-output.mjs` 的精确 HTML inventory 已扩为 13 页；六个 Entry 页面均按既有模板保留一个 inactive Reader Request，其余七页不得出现。每页仍恰有一个 inactive Newsletter。
- 四个 draft 的 `nameZh` / `pinyin`、正文、反向证据关系和资产均为空，因此不扩展可见 CJK 字符或 Hero inventory；Explore/Collections 固定候选架仍只包含既有两个 Entry 与一个 Collection。
- 当时输出合同已由完整门禁验证：13 个 `noindex, nofollow` HTML、42 Hero、10 WOFF2、0 XML、0 客户端 JavaScript；25 个测试文件/321 项测试与 Astro 81 个文件零诊断通过。没有执行四个新增页面的浏览器/键盘/缩放人工 QA。

### 12.16 Privacy public 纯 builder 补齐

- 2026-09-07 Project owner 授权继续不涉及账号的本地开发；在既有 published 批次未提交工作树上，补齐 `/privacy/` 的精确 SEO 描述与固定 Sitemap 成员。只修改 `src/site/seo.ts`、`src/site/release-artifacts.ts` 与对应两份测试，不改页面、Layout、intent、runner、endpoint、配置、依赖、正文或视觉资产。
- Privacy 输出普通 `WebPage` 和同源 canonical/text-only OG；Sitemap 五静态路径各一次，缺失/重复/非规范 Privacy 路径失败，Privacy 不进入 RSS。沿用 synthetic fixture origin，仅为内存测试输入，不代表已控制域名或已授权站点身份。
- 与 006 的 U5A 一起完成定向 5 文件/98 测试、完整 26 文件/433 测试、Astro 83 文件零诊断和 14 页 noindex build/output verifier；当前输出仍为 112 Hero、10 hash-locked WOFF2、零 XML、零客户端 JavaScript。未扩展 review/public 输出策略，也未新增宽松路径规则。
- 追溯与披露保持不变；既有构建期 Schema、图/资产与 output 诊断仍执行；复用精确路径/清单职责和匹配测试，不增加额外兼容分支。未执行浏览器或最终 QA、服务、真实网络、Git 写入、推送或发布。

### 12.17 2026-09-07 完整首发内容的本地开发预检

- 授权与范围：Project owner 同意本地整站预检及直接问题修复，包括临时 preview/browser 和结束清理；延续既有未提交工作树，不改正文、内容状态/日期、资产、Schema、运行配置、依赖或真实服务。此处为 dirty-source nondeployable 开发证据，不是最终发布 QA 或新增 owner 视觉批准。
- 修复：手机 Entry 的桌面高优先级 `16:9` 被精确覆盖为 `4:5`；中小屏标题回归文档流，桌面 H1 上限 `8ch` 调整为 `12ch`，避免长标题超出 Hero；桌面叠图 identity 增加局部暗底，修复钟馗浅图上的白字对比。正文落实现有 `68ch` 上限，Privacy 用已定义 `--space-4` 恢复段落/地址间距。桌面和移动导航均将精确当前页标为 `page`、所属栏目标为 `location`，不相关页不标记。
- 自动层：新增 6 项响应式回归与 21 项导航正反例；实际 14 页继续经过导航 output oracle，覆盖详情误称当前页、错分区、遗漏状态和仅移动导航漂移。完整 check 最终通过 26 文件/460 测试、Astro 83 文件零诊断和既有 14 页/112 Hero/10 WOFF2/零 XML/零客户端 JavaScript 门禁。暗底第一版的隐藏伪元素写法被既有安全校验拦截，改为只在桌面断点生成后通过，没有放宽安全规则。
- 浏览器：内置连接失败后改用独立 profile 的 Windows Edge 152.0.4191.66 headless；沙箱内 GPU 启动失败，经工具审批在沙箱外运行同一本机预检。最终 390/768/1024/1440 × 900 共 56 组均无横向溢出、图片加载失败、页面运行时异常，字体状态为 loaded，main/H1 各一。逐路由首屏及重点正文/来源截图复核没有新遮挡；六篇手机 Hero 为 4:5，1440 正文从约 896.7px 收敛到 625.6px（当前字体的 68ch），Privacy 段落/地址均为 16px 上间距。1024《促织》标题组底边约 519.5px、图片底边 666px，已分离。
- 路径与边界：静态 246 内链、20 片段链接、76 ARIA 引用可解析且 ID 唯一，两个 Guided/Browse、Featured 及反向合集关系正确；390px 浏览器通过 DOM 链接激活实走两条阅读环并回到各自合集，共 10 次跳转。此方法不证明真实键盘或触控。页面资源未观测到外部请求，唯一 HTTP 错误为浏览器自动请求尚不存在的 favicon（404），暂不新增品牌图标资产。
- 配套与未验证：Source/Claim/术语、原图/字体/manifest、人工资产审核和 alt/caption/credit/AI disclosure 不变；Schema、内容/视觉图、inert 交互与输出安全门禁保留。标题在流内和图片明暗独立的对比保护有职责说明。未执行非默认视觉再生产、真实键盘/读屏器、200% zoom、偏好/禁用 JS/慢或失败资源、实际 fallback face、LCP/CLS、跨平台、真实供应商、public artifact、clean-source receipt 或发布验收；不因本轮通过关闭这些门禁。preview 与自有 Edge 已关闭，未提交、推送或部署。

### 12.18 2026-09-08 本地无障碍与异常加载预检

- 目标与授权：Project owner 同意在既有 dirty-source review 上继续本地预检和直接缺陷修复；允许临时 preview/隔离浏览器并在结束停止。不改内容/状态/日期、资产、Schema、外部服务、依赖或运行配置，不提交、推送或部署。
- 范围：14 页在 390/1440 CSS px 下分别记录正常、禁用 JavaScript、reduced-motion、图片失败、字体失败的内容/链接/尺寸/可见性；两个合集补 768px 图片失败检查。以浏览器默认按键检查 skip link、菜单、Tab/Shift+Tab、Enter/Space、焦点可见与离开菜单后的遮挡，保持 inactive 控件原有 disabled 合同。
- 修复与回归：实际阻断两种 Hero 图片格式后，两个合集出现浅字浅灰底；只给合集图片容器增加既有 `--realm-bg` 备用底色与职责注释，不改变原图、alt、图注、披露或 Home/Entry 中性底色。新增底色回归先失败、修复后通过；另补 reduced-motion 关闭平滑滚动/缩短过渡且不增加布局属性的有限静态护栏。定向 13 项与最终完整 26 文件/462 测试通过；Prettier、ESLint、Astro 83 文件零诊断、14 页 noindex/output verifier、112 Hero、10 WOFF2、零 XML/客户端 JavaScript 继续通过。
- 故障与偏好证据：Windows Edge 152.0.4191.66 headless、新 profile、禁用缓存，最终 142 组（14 × 2 × 5 + 2）无正文可见文本/链接变化、横向溢出、图片容器尺寸变化或页面运行时异常。20 组有图页面实际记录 Image 加载失败且 naturalWidth 为 0；28 组字体阻断均有 Font 失败，使用字体 error、无 loaded 自托管 face。脚本开/关用独立空白控制页的普通 inline script 验证；reduce 模拟下 matchMedia 为 true、scroll-behavior 为 auto、过渡为 0.01ms。页面资源无外部请求；唯一 HTTP 错误仍为默认 favicon 404，不算已解决。
- 视觉与字体证据：已复核两个合集三档失败截图、Home/Entry 失败样本、无脚本首页、字体失败后的文章正文/来源与 SC/TC 样张。合集失败底色实测 rgb(16,23,22)，H1 为 rgb(242,243,239)，此单一组合计算对比约 16.30:1，不声称全站对比通过。CSS 平台字体查询在所采文本节点实际命中 Microsoft YaHei UI、Palatino Linotype、Microsoft YaHei 与 Microsoft JhengHei；这是当前 Windows 的抽样 fallback 证据，不是所有字形/字重/平台验收。字体失败导致个别手机图片在文档流中纵向位置变化，尺寸保持，不把它记为零 CLS。
- 键盘证据：14 页 × 两视口 × 脚本开/关，共 56 组导航预检；以 CDP 按键默认动作而非 focus()/click()/DOM 伪事件验证首个 skip → main → 后续内容、精确四导航顺序、手机 Enter/Space 开关与反向回到 summary、Tab 离开菜单、2px focus 及局部遮挡采样，四组本地 Explore 链接 Enter 导航通过。初版过早读取平滑滚动与链接边界盒产生误报，改为新文档起点、等待滚动稳定及实际文本行采样；样张正文 header 被误判为全站 header 后收窄检查器，并单独重跑其四组通过。网站键盘实现无需变更。完整正文/所有外链/页尾 Tab 遍历及读屏器不在此证据范围，未激活外部来源或真实表单。
- 配套与收口：Source/Claim/术语、资产/字体追溯和人工审核记录不变；Schema/图/资产/inert/output 安全诊断保持，非默认视觉再生产不适用。命令和临时进程身份见 DEV_WORKFLOW；本次 preview 与自有浏览器均已停止、精确 PID/4321 回查无进程或监听。未改依赖/配置、Git 或部署状态；不关闭 M6 最终 clean-source public artifact、真人键盘/读屏器、真实 200%、慢资源、LCP/CLS 或跨平台门禁。

### 12.19 本地 public assembly 接线（2026-09-08）

原 U4B 在 [013](013-public-artifact-assembly.md) 进入并完成本地实现：owner 网页创建的空 Vercel 项目与稳定域名已核截图，显式 public origin、严格 published 路由、全站 SEO、可见 Article 日期、Sitemap/RSS/robots、独立 output verifier 已接线。review 字体样张移至 `src/review/type-specimen.astro`，按 intent 注入原 URL，public 不生成样张。review/public 共用原有逐页 Hero 归属、响应式候选与披露门禁，review 的 noindex/零发现 metadata 限制不放宽。

本批仅为未提交源上的本地诊断，不关闭 011 的完整候选、Beta 文案/位置、hosting Privacy、M5-U4/U5、clean-source QA/receipt、受保护预览或生产授权。准确验证结果以 013 与 DEV_WORKFLOW 为准，前述阶段快照保留当时事实。
...
- 已满足：显式 review、纯 published-only release/public 最小投影、固定 review-only 候选架、M4 的原 7 页加 direct-only specimen 共 8 页 noindex 历史输出、两篇编辑候选、三套 current Hero、自托管 4 份英文与 6 份 CJK WOFF2 自动门禁、样张 20 个精确组合与实际 HTML/CSS/pinyin/mixed/head/direct-only oracle、CJK 语言/required/fallback 边界、Source、真实 release 空状态、About/身份、内部链接、静态键盘语义、零客户端 JavaScript、24 个最终三档浏览器组合、12 个 Hero art-direction 组合，以及纯 SEO/artifact 负门禁。M5-U3 的 Privacy、四个 draft Entry 与后续 Liaozhai draft Collection 使当前 review 合同增至 14 页；它们不改变前述历史验收范围或 public 投影。
- 尚未满足：M5-U4 的 Buttondown 账户级配置/真实 action/合成订阅联调、Tally 草稿发布与精确 supplier transport/link/联调，以及 U5 Plausible 接线；M6 的最终 artifact 发布资格、真实键盘/200%/媒体偏好、禁用 JS、字体/图片失败、LCP/CLS、实际 fallback 与支持平台验收；M7 的 Public Beta 生产发布、发布后基线与目标读者验证。完整 6+2 published inventory 已获独立批准，Privacy 纯 builder 与 U5A 离线阅读判定也已补齐，不得再列为尚未开发，但都不等于 public artifact 已完成。
- M5-U2 provider-neutral 合同/Fake 与 [M5 详细需求](006-external-interactions.md) 中 U3 的公开隐私、页面文案和 inert review UI 已完成；Buttondown 审核已通过只关闭等待审核门槛，U4 账户准备仍不等于接线完成。外部交互下一停点须分别授权 Buttondown 账户级配置/真实 action/合成订阅，以及 Tally 发布/hosted link/合成数据/回查/删除。内容侧当前为 6 个 published Entry 与 2 个 published Collection，六篇目标公开日期为 2026-09-10；四个新增 Entry 均有证据消费关系、正文、人工双语审核和 approved/current Hero 绑定，Liaozhai Collection 也已有独立 Hero。两个三篇 Collection 路径已经接线；013 已完成真实 origin/public assembly 的本地接线；下一内容停点为完整发布候选准备。M6 再承接完整内容、public artifact assembly、最终 release-candidate QA、receipt 与受保护预览；M7 Public Beta 上线后再执行 R2a/R2b。本批不自动授权真实联调、后续 Git 写入或 Vercel 部署。
- 是否可以关闭：M4 总需求可以按本地产品实现边界关闭；M6/M7 发布责任仍保持 fail closed，`001-mvp-foundation` 仍不能关闭。

## 014 版面调整的接续

2026-09-08，[014](014-editorial-design-refinement.md) 在当前 6+2 published inventory 上调整首页、两个目录与 Entry 首屏。新增图位继续使用原 owner 的 approved/current Hero 和完整披露；目录顺序与单项唯一链接、review/public 隔离、完整标题/摘要/来源合同保持。图位门禁改为 11 页 / 19 图位 / 8 families / 112 唯一衍生文件；此前 9 个 Hero 页面和桌面叠字属于旧版历史证据。新版验收结果由 014 记录，未改变 M6/M7 发布边界。

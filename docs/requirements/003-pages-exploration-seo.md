...
- `zhong-kui`、`chinese-underworld-guide` 和 `chinese-underworld` 都是 non-published `editorial-review`；两篇 Entry 已有 opening、summary、正文、来源/Claim、`bilingual-approved` 术语与 fact-check 日期，四条馆藏标题 locale 也已核定为 `zh-Hant`。另有 `ten-kings`、`liaozhai-reading-guide`、`painted-skin`、`fighting-cricket` 四个无资产的 non-published `draft` Entry；Ten Kings 已有正文与 2 Source / 3 Claim / 1 Terminology 反向消费关系，其余三个的 body/反向数组仍为空。它们不进入固定 Explore/Collections review candidate。钟馗 earliest-known 双证据仍明确为 `null`，不由不充分材料补齐。
...
- 当前原有两篇 Entry 已形成完整候选叙事与来源层级，Terminology 已为 `bilingual-approved`，四条馆藏标题 locale 与 CJK 静态生产链已闭合；钟馗 earliest-known 保持空。另有四个无资产的 `draft` Entry；只有 Ten Kings 已建立 2 Source / 3 Claim / 1 Terminology 关系与证据受限正文，其余三个只有稳定 owner 与限域证据对象。M6 完整内容状态、public artifact、字体/故障/跨平台 release QA 与发布资格仍未闭合，因此不等于发布就绪。
...
- M4-U2 当时的钟馗候选缺少多数正文输入，因此 U2 只能渲染真实存在的字段并省略不存在的内容区块，不得从 Claim statement 自动拼成文章或用 lorem/AI 草稿填充。后续两篇 Entry 已形成当前 `editorial-review` 正文候选；2026-09-04 新增的四篇仍为 draft owner，其中 Ten Kings 已形成证据受限首稿，其余三个为空。它们都仍须在 M6 经过各自完整编辑/证据门禁和状态决定。

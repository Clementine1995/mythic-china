...
| 层 | MVP 推荐 | 选择原因 | 当前状态 |
| --- | --- | --- | --- |
| 页面与构建 | Astro 7 静态模式 + TypeScript strict | 内容型、默认静态、可在有事实触发时局部升级 | M4 review 基线是原 7 个内容/功能页与一个 direct-only type specimen；M5-U3 又增加 Privacy；四个新增 draft Entry 依既有动态路由生成 direct-only review 页面，当前共 13 个 `noindex, nofollow` 页面；public intent 尚未开放 |
| 内容 | Entry 使用 Markdown；Collection、Source、Claim、Terminology 使用 YAML；一对象一文件 | Git 可追踪、正文与结构化记录职责明确、避免先建 CMS | 当前为 6 Entry（2 `editorial-review` + 4 `draft`）、1 Collection、14 Source、19 Claim 与 5 Terminology；published inventory 仍为 0/0 |

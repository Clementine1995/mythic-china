...
- 阶段：M1 已冻结为工程参考；M2 与 M3 已完成并形成历史基线 `f258227`、`c606f5`。M4-U1/U2 的历史基线为 `5f327b6`，M4-U3 为 `e94eaca`，首个纵切片 release-readiness 为 `eb6e20c`；后续检查点包含 `a3194d0`、`d60691a`、CJK 字体提交 `9434a76` 与 M4-U5/功能页提交 `e2893d1`。经 2026-09-02 项目总检与范围重定界，M4 本地产品实现由 Project owner 提交为 `3983bee`；M5-U1–U3、U4 账户准备快照与第二 Collection 决策包随后提交为 `3fa46d5`。Project owner 后续确认 007 推荐、Liaozhai 任笃行 2016 主/张友鹤 2011 对校路线及《促织》青柯亭单见证路线，并逐次授权四篇研究、证据闭合、最小 draft Entry owner 及证据已闭合子集的 Source/Claim/Terminology 物化；这些结果与 Buttondown 审核事实同步已形成本地检查点 `9914dd3`。Project owner 又批准 Ten Kings 单篇纵切片并授权本地提交；随后另行授权 Fighting Cricket 单见证纵切片，并在中文概要复核后确认内容符合预期、授权本地提交。两篇均已进入 `7c0cb3f`，该检查点随后由 Project owner 推送；Guide 批开始时三个本地 ref 对齐。Project owner 于 2026-09-05 又通过中文概要确认 Liaozhai Reading Guide 内容符合预期，并单独授权验证通过后本地提交；该纵切片已提交为 `119c01c`；Project owner 随后确认已 push。009 开始前，本地 HEAD、`main`、`origin/main` 与 GitHub 服务器只读查询的 `main` 均为 `119c01c2937e4598e66c0e5886ddc917e78af143`，工作树当时干净；该提交尚未处理图片、Collection 关系、状态、public artifact、外部服务或发布。当前修改位于该基线之上，本批代理未执行 fetch 或 push；tracking ref 不单独证明服务器端状态。
- 当前实现：默认 review 构建在 M5-U3 的 9 页历史基线上新增四个 direct-only draft Entry 路由，第二 Collection 接线后现按精确清单生成 14 个 `noindex, nofollow` 页面与既有 42 个 Hero AVIF/WebP。全站 Footer 恰有一个 disabled/inactive Newsletter，六个 Entry 在 Sources 与继续阅读路径后各有一个 disabled/inactive Reader Request；它们只显示用途、字段/独立 consent、未开放状态与本地 Privacy 链接。M5-U2 的三个 `src/services/` 仍只提供 strict provider-neutral 合同与零网络 Fake。站点与仓库中没有 form、Buttondown action/link、Tally hosted link、Plausible 脚本、配置、存储或真实 transport。当前内容 inventory 为 6 篇 Entry（2 `editorial-review` + 4 `draft`）、2 个 Collection（1 `editorial-review` + 1 `draft`）、14 份 Source、25 份 Claim 与 6 份 Terminology；published Entry / Collection 仍为 0/0。四个新增 `draft` Entry 均有证据受限草稿；修订前锁定稿及 010 当前四篇改动单元和两条 `contentNote` 均已通过 Project owner 双语确认，未改单元保留历史批准。`ten-kings`、`fighting-cricket`、`liaozhai-reading-guide` 与 `painted-skin` 分别消费 2/3/1、1/3/1、1/2/1 与 2/7/1 的 Source/Claim/Terminology。六份 Terminology 继续为 `bilingual-approved`，Painted Skin 短译 Claim 继续为 `verified`。四篇已按 009 纳入两个合集；related 关系与视觉字段仍为空，M4 public SEO/artifact 纯 builder 仍未被 Layout/路由/endpoint 接线。
- 编辑/视觉状态：两篇首发 Entry 与 Chinese Underworld Collection 的 `editorial-review` 形态已获 Project owner 阶段性接受；Project owner 先以 `Project owner (user-confirmed)` bilingual reviewer 身份批准两份 Terminology，2026-09-05 又确认三篇纠偏稿通过人工双语审核，2026-09-06 再确认 Painted Skin 及 010 四篇聚焦差异通过人工双语审核。当前确认覆盖受影响英语单元、对应中文含义和两条 `contentNote`；六份 Terminology 仍为 `bilingual-approved`，四条馆藏中文标题的正字 locale 仍已核定为 `zh-Hant`。Collection Hero 与 Guide Hero 均已完成 Project owner 选图、Guide/Collection 各自的账户与权利确认、五类人工审核、exact-canvas master、production record、approved/current manifest、repository source 和所属内容的 versionless `assetId` 绑定；Liaozhai Collection 仍为无 Hero/Featured 的 `draft`，Entry/Collection 状态没有因此提升。
- 当前验证：2026-09-06 四篇 AI 专业文案修订与文章级内容提示实现后，固定 Node/Corepack 的完整 `pnpm run check` 通过 Prettier、ESLint、25 个测试文件/369 项测试、Astro 81 个文件零诊断、14 页静态 review build 与 output verifier；实际输出仍为 42 Hero、10 个 hash-locked WOFF2、零 XML、零客户端 JavaScript。输出 oracle 精确核对只有 Fighting Cricket 与 Painted Skin 显示对应 `contentNote`，且位于署名之后、Opening/Quick Answer/正文之前；四篇 `null` Entry 不显示提示，并拒绝提示及子节点的隐藏、警报、内联样式、嵌套交互、重复结构和错误顺序。此前 351/321 项测试和最初接线验证保留为历史证据。2026-09-02 的 24 个 M4 实际视口组合、12 个 Hero art-direction 组合与 Project owner 当时 8 页判断同样只属历史；新增页面仍未做真实浏览器、键盘、缩放或人工视觉验收。非默认 `visual:build:check` 不适用，因为本批没有修改图片、manifest 或 master。
...
- 版本边界：2026-09-02 进入 M5 前只读复核确认 HEAD 与本地 `main` 为 `3983bee91ada4a286613ec702a8009a4f528af3f`，后续本地 `origin/main` tracking ref 由外部 push 对齐；Project owner 又把 M5 与 007 状态提交为 `3fa46d5f85c43a5278e15ca6b0630d724439acc9`。四篇研究、证据最小物化及相关状态同步提交为本地检查点 `9914dd304467118b2c32f7f8ac192cc00344fb92`；Ten Kings 内容纵切片随后进入 `a5a15a5c74f7a18a8c15e0cdd5ac1e44574151c2`，Fighting Cricket 纵切片在中文概要确认与提交授权后进入 `7c0cb3fbea2c6175f01790160fe2460bb658e0f8`。Project owner 随后推送该检查点；Guide 批开始时三个本地 ref 对齐。Liaozhai Reading Guide 在 2026-09-05 获中文概要确认与独立本地提交授权后进入 `119c01c2937e4598e66c0e5886ddc917e78af143`，Project owner 随后确认已 push。本批通过 GitHub branches/main API 独立核得服务器 `main` 同一 SHA，与本地 HEAD、`main`、`origin/main` 一致；不以 tracking ref 代替服务器证据。当前修改位于该基线之上，包含 009、三篇纠偏/审核状态与 Painted Skin 单见证草稿；未执行 fetch、Git 写操作、建分支或 worktree。
- 工作名称：`Mythic China`；目录名与未来包名使用 `mythic-china`。当前权威日期为 2026-09-06。
...
- 三篇首稿编辑复核：2026-09-05 先按既有 Source/Claim/Terminology 为十王、促织和聊斋导读完成一轮 AI 编辑修订；当批完整检查通过 25 文件/321 测试、Astro 81 文件零诊断与 13 页 review 构建。后续逐字复核发现并纠正三组 verified Claim 与正文的证据偏差，同时收紧 1679 年代归因和 `zhiguai` 语境；此前 AI 编辑结论及审核包不再作为英文与证据一致性已通过的依据。三篇仍为 `draft`；Project owner 随后确认纠偏后审核包通过人工双语审核，三份术语提升为 `bilingual-approved`。没有新增来源或页面视觉验收。详见 008 第 14.5–14.6 节。

- 合集接线：2026-09-05 完成两个 3 篇阅读路径；阴间按指南 → 十王 → 钟馗，聊斋按导读 → 画皮 → 促织。新增 `/collections/liaozhai/` 本地评审页，既有模板派生六篇反向入口；关系校验加固后的完整检查通过 351 项测试与 14 页输出验证。状态仍为阴间 editorial-review、聊斋 draft，published 0/0；Painted Skin 修订前锁定稿及 010 当前四篇改动单元均已通过聚焦双语确认，四篇仍缺目标读者、视觉与状态审核。见 [009](docs/requirements/009-collection-reading-paths.md)。
- 目标读者文稿审核：[`010-four-entry-reader-review.md`](docs/requirements/010-four-entry-reader-review.md) 的修订前 R1 已被当前重建版本取代且不得分发。聚焦双语确认后，协调者 Markdown 与唯一参与者 HTML 已从当前 build 重生成；20/20 输入哈希、4 篇可见文本、2 条内容提示、结构、离线性、唯一 ID/ARIA/片段引用与严格 UTF-8 检查通过。当前下一停点是独立读者可用后执行 R2a 非计入试跑；目前非计入试跑与正式真人反馈均为 0，目标读者门禁保持未通过。
...
3. **M6 内容与 public artifact assembly**：Project owner 已确认 [`007-second-collection-decision.md`](docs/requirements/007-second-collection-decision.md) 的 Liaozhai 第二 Collection 方向、3+3 分配、公开顺序、Zhong Kui Featured 与《促织》保留；两个合集路径已按 009 接线。四个 draft Entry 现均有证据受限草稿；此前锁定稿及 010 本轮改动单元与两条内容提示均已通过双语确认，当前 R1 已重建并通过静态检查。Painted Skin 采用上海图书馆 1766 青柯亭单见证，消费 2 Source / 7 Claim / 1 `bilingual-approved` Terminology；短引自译 Claim 已为 `verified`，并保存原文、版本、locator、译者/日期与审核记录。当前全库为 14 Source / 25 Claim / 6 Terminology。任/张现代校勘本、CText/Giles、现代译本和电影直接比较不进入 Painted Skin 的当前 MVP。找到独立人选后，下一内容停点是 1 位非计入样本的 R2a 试跑，随后再做 5–8 位正式英语读者审核。资产与状态审核仍是独立后续门禁。图片、状态与 public artifact 仍须分别授权；Project owner 作出 `published` 决定后，才确认 HTTPS origin 并实施 public artifact assembly。当前结果不表示已部署或上线。
...
| --- | --- | --- | --- | --- | --- |
| 本地应用、文档与原型工作区 | `F:\codex-project\mythic-china` | M4 基线为 `3983bee`；M5-U1–U3、U4 准备快照与 007 基线为 `3fa46d5`；四篇研究与证据最小物化检查点为 `9914dd3`；Ten Kings 与 Fighting Cricket 纵切片已进入 `7c0cb3f`，Project owner 推送后 Guide 批开始时三个本地 ref 对齐。Liaozhai Reading Guide 检查点 `119c01c` 已由 Project owner 确认推送；2026-09-05 本地三个 ref 与独立 GitHub 服务器查询的 `main` SHA 一致，核验过程未 fetch 或写 Git | 固定 Node、pnpm、CJK 生产、构建、验证与 Git 命令见 `DEV_WORKFLOW.md` | 6 Entry / 2 Collection / 14 Source / 25 Claim / 6 Terminology、既有资产/字体 inventory、三个 provider-neutral service/零网络 Fake、14 页 inert review UI 与 Privacy；四篇候选均已有证据受限 draft，修订前锁定稿及当前改动单元均通过双语确认，当前 R1 已重建并通过静态检查，六份 Terminology 均为 `bilingual-approved`；外部已有审核通过的 Buttondown 账户及 Tally Free 未发布草稿，但仓库无凭据、action/link、provider 配置、数据库或外部写入口 | M4 review/public 纯门禁、M5-U2 内存合同、M5-U3 静态 UI/output 验证，以及 007/008/009 的方向、研究、四篇证据受限纵切片与两个 Collection 成员路径；不含真实联调、图片、`relatedEntryIds`、状态提升、M6 public artifact assembly、最终 release-candidate QA、clean-source receipt、远端预览或发布 |

...
- 阶段：M1 已冻结为工程参考；M2 与 M3 已完成并形成历史基线 `f258227`、`c606f5`。M4-U1/U2 的历史基线为 `5f327b6`，M4-U3 历史基线为 `e94eaca`，首个纵切片 release-readiness 基线为 `eb6e20c`；后续检查点包含 `a3194d0`、`d60691a`、CJK 字体提交 `9434a76` 与 M4-U5/功能页提交 `e2893d1`。经 2026-09-02 项目总检与范围重定界，M4 本地产品实现已完成，并由 Project owner 提交为本地基线 `3983bee`；进入 M5 前工作树干净。当前工作树承接 M5-U1 需求/权威文档、M5-U2 provider-neutral 合同/Fake、M5-U3 inert review 页面、U4 账户准备现场同步与第二 Collection 只读研究决策包；public artifact、发布候选 QA、远端预览与发布仍未完成。
- 当前实现：默认 review 构建在原 8 页基础上新增 `/privacy/`，现为 9 个 `noindex, nofollow` 页面与 42 个 Hero AVIF/WebP。全站 Footer 恰有一个 disabled/inactive Newsletter，两个 Entry 在 Sources 与继续阅读路径后各有一个 disabled/inactive Reader Request；它们只显示用途、字段/独立 consent、未开放状态与本地 Privacy 链接。M5-U2 的三个 `src/services/` 仍只提供 strict provider-neutral 合同与零网络 Fake。站点与仓库中没有 form、Buttondown action/link、Tally hosted link、Plausible 脚本、配置、存储或真实 transport。真实内容 inventory 仍为 0 published Entry / 0 published Collection；M4 public SEO/artifact 纯 builder 仍未被 Layout/路由/endpoint 接线。
...
- 当前验证：M5-U3 固定 Node/Corepack 的定向测试通过 3 个文件/79 项测试；完整 `pnpm run check` 通过 Prettier、ESLint、25 个测试文件/320 项测试、Astro 81 文件零诊断、9 页静态 review build 与 output verifier。新增 HTML5 DOM oracle 逐页验证 Newsletter、两个 Entry 的 Reader Request、Privacy、顺序/唯一性、disabled/inactive 状态、稳定 page ID、零 provider link/form/script 和零假成功；构建为 9 页/42 Hero/10 hash-locked WOFF2/零 XML/零客户端 JavaScript。2026-09-02 的 24 个 M4 实际视口组合、12 个 Hero art-direction 组合与 Project owner 当时 8 页判断仍是 M4 历史证据；U3 新页面和交互入口没有进行真实浏览器/键盘/缩放验收。非默认 `visual:build:check` 的既有证据为 11 个 local master、7 份 current responsive rendition 与 50 个实际 AVIF/WebP 输出，本批不适用。
...
- 版本边界：2026-09-02 进入 M5 前只读复核确认 HEAD 与本地 `main` 为 `3983bee91ada4a286613ec702a8009a4f528af3f`，工作树、暂存区与未跟踪文件均为空；当时未 fetch 的本地 `origin/main` 仍为 `e2893d14d4960f71fe75bd240971aaa88656511c`，本地显示 ahead 1。M5-U2 实施期间，reflog 显示 `origin/main` 于 2026-09-02 14:04:40 +0800 由外部 push 更新到 `3983bee`，当前 HEAD、`main` 与 tracking ref 对齐。本批没有执行 fetch、add、commit、push、amend、分支或 worktree；tracking ref 仍不能替代服务器端只读复核，精确身份须按 `DEV_WORKFLOW.md` 在操作前复核。
- 工作名称：`Mythic China`；目录名与未来包名使用 `mythic-china`。当前权威日期为 2026-09-03。
...
2. **M5 外部交互边界**：[`006-external-interactions.md`](docs/requirements/006-external-interactions.md) 的 U2 provider-neutral 合同/Fake 与 U3 inert Newsletter、Reader Request、Privacy 及完整本地门禁已完成。Buttondown `mythic-china` 已创建但仍在人工审核；Tally Free 账户与未发布 Reader Request 草稿已准备。两者均未接入站点、未提供 action/hosted link、未导入订阅者、发送邮件或处理提交，Plausible 留 U5；下一停点是等待 Buttondown 审核结果，以及 U4 对 Tally 草稿发布、精确 action/link、合成数据写入、回查和删除步骤的独立授权。当前仍不渲染可提交表单、不接真实网络，也不让这些准备状态反向改写静态内容和发布状态合同。
3. **M6 内容与 public artifact assembly**：完成 6 篇内容、至少 2 个合集页和全部视觉内容包；第二 Collection 的只读候选比较、推荐 3+3 分配与待决策项见 [`007-second-collection-decision.md`](docs/requirements/007-second-collection-decision.md)，该推荐尚未获 Project owner 确认。在内容拥有的编辑、证据、术语、关系、批准资产与无障碍文案门禁满足后，由 Project owner 逐项作出 `published` 决定。随后确认真实 HTTPS origin，并接入原计划 M4-U4B 的 public intent/runner、六类页面 metadata、Sitemap/RSS endpoint 与独立 public output verifier。`published` 表示可以进入 public artifact，不表示已部署、远端预览或生产上线。
...
M2/M3-U5 已建立本地静态应用、视觉资产合同和非默认生产复核链路：`Entry Markdown / 结构化 YAML + visual brief / production record / versioned manifest -> 严格 Schema -> 纯内容图与视觉资产图/文件校验 -> 显式 current resolver -> published-only release / review 投影 -> Astro review 静态路由 -> dist/`。M5-U2 另建立不进入页面/构建输出的交互合同链：`unknown input -> strict provider-neutral Schema/allowlist -> redacted result -> injected zero-network Fake`。M5-U3 在 9 页 noindex review 输出中加入 `inactive Newsletter / Reader Request -> local Privacy notice -> fail-closed DOM oracle`；三类 service 仍没有 provider mapping、配置、存储或网络，public 纯 builder、Hero/字体、CJK 与内容状态未被改写。`prototypes/m1-home.html`、`prototypes/m1-collection.html`、`prototypes/entry-reader-flow.html`、`prototypes/m1-review-board.html` 与 `prototypes/m1-type-specimen.html` 继续组成冻结的 M1 独立原型，不属于应用构建输入。目标体验与内部生产仍必须分开理解：
...
| --- | --- | --- | --- | --- | --- |
| 本地应用、文档与原型工作区 | `F:\codex-project\mythic-china` | HEAD、本地 `main` 与当前 `origin/main` tracking ref 均为 M4 基线 `3983bee`；进入 M5 前干净，当前工作树含 M5-U1–U3、U4 账户准备同步与第二 Collection 研究文档。tracking ref 在 U2 期间由外部 push 更新，本批未执行 Git 写入或 fetch；服务器端状态仍须另行复核 | 固定 Node、pnpm、CJK 生产、构建、验证与 Git 命令见 `DEV_WORKFLOW.md` | 既有内容/资产/字体 inventory、三个 provider-neutral service/零网络 Fake、9 页 inert review UI 与 Privacy；外部已有审核中 Buttondown 账户及 Tally Free 未发布草稿，但仓库无凭据、action/link、provider 配置、数据库或外部写入口 | M4 review/public 纯门禁、M5-U2 内存合同、M5-U3 静态 UI/output 验证与本轮只读决策输入；不含真实联调、新内容对象、M6 public artifact assembly、最终 release-candidate QA、clean-source receipt、远端预览或发布 |
...
- `docs/requirements/`：`001` 保存 MVP/M1–M7 总合同，`002` 保存 M3 视觉资产管线合同，`003` 保存 M4 页面、探索与 SEO 的详细合同、状态投影、实施单元和验收门禁，`004` 保存首个纵切片的编辑、Collection/Guide Hero 候选检查点及后续生产闭环与生产字体状态，`005` 保存已确认但尚未实施的本地化内容试点合同，`006` 保存 M5 外部交互、隐私、供应商与 Mock/真实联调边界，`007` 保存第二个 MVP Collection 的只读研究比较、推荐分配与 owner 决策停点；目标合同、研究建议和执行快照均不自动代表当前实现或已交付状态。

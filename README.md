...
- 版本边界：2026-09-02 进入 M5 前只读复核确认 HEAD 与本地 `main` 为 `3983bee91ada4a286613ec702a8009a4f528af3f`，后续本地 `origin/main` tracking ref 由外部 push 对齐；Project owner 又把 M5 与 007 状态提交为 `3fa46d5f85c43a5278e15ca6b0630d724439acc9`。四篇研究、证据最小物化及相关状态同步提交为本地检查点 `9914dd304467118b2c32f7f8ac192cc00344fb92`；Ten Kings、Fighting Cricket 与 Liaozhai Reading Guide 分别进入后续检查点。009、Painted Skin、四篇当前文案与 010 审核基线已由 Project owner 提交为 `f81b49fd1c288aed6c552c2e8d9926ec5a09e86c`。2026-09-07 路线修改开始时，本地 HEAD、`main` 与本地 `origin/main` tracking ref 对齐到该提交且工作树干净；本轮未 fetch，因此不以 tracking ref 证明服务器端状态。路线文档实施与验证阶段只有文档修改，未建分支或 worktree；Project owner 随后授权将同一批文档创建本地提交，该授权不包含 fetch、push 或发布。
- 工作名称：`Mythic China`；目录名与未来包名使用 `mythic-china`。当前权威日期为 2026-09-07。
...
- 合集接线：2026-09-05 完成两个 3 篇阅读路径；阴间按指南 → 十王 → 钟馗，聊斋按导读 → 画皮 → 促织。新增 `/collections/liaozhai/` 本地评审页，既有模板派生六篇反向入口；关系校验加固后的完整检查通过 351 项测试与 14 页输出验证。状态仍为阴间 editorial-review、聊斋 draft，published 0/0；Painted Skin 修订前锁定稿及 010 当前四篇改动单元均已通过聚焦双语确认。四篇仍缺视觉与状态审核；目标读者反馈为 0，按 011 留到 Public Beta 上线后。见 [009](docs/requirements/009-collection-reading-paths.md)。

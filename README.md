...
- 当前验证：固定本机 Node/Corepack 下完整 `pnpm run check` 通过 21 个测试文件/201 项测试，Astro 69 文件零诊断，默认 review 构建通过 7 页/42 个 Hero 图片/10 个 hash-locked WOFF2/零 XML/零客户端 JavaScript，并通过 CJK 生产来源、构建后 CSS、有效 glyph cmap 与 parse5 HTML5 实际树语言边界门禁；非默认 `visual:build:check` 的既有证据为 11 个 local master、7 份 current responsive rendition 与 50 个实际 AVIF/WebP 输出复核，本批未改视觉链，未重跑该非默认入口。
- 视觉/字体状态：M1 原型视觉皮肤仍未获批准；M4-U2 页面方向与此前 system fallback 三档视口已确认。Collection Hero 与 Guide 自有 Hero 的生产链均已闭合；4 份上游未修改英文 WOFF2 及 Source Han Sans 2.005R 派生的 SC/TC × 400/500/600 六份静态 WOFF2 已通过中央 alias/token、OFL/RFN/FONTLOG、精确 `unicode-range`、hash/name/cmap 与实际 HTML 字符门禁。正式字体浏览器效果、慢加载、真实键盘/缩放/媒体偏好、性能、跨平台 fallback 及 M4-U5 最终视觉尚未批准。
...
1. **首个纵切片与 M4-U5 预检**：Collection/Guide Hero、双语术语、四条馆藏标题 locale，以及 CJK 字符输入、六份子集、RFN/FONTLOG 与 cmap 静态门禁均已闭合。下一步需要 Project owner 另行授权服务/浏览器，在 noindex review 直达页与真实空索引判断正式英/CJK 字体命中与页面效果，并完成键盘、200% 缩放、禁用 JavaScript、reduced motion、慢字体、图片失败、性能、跨平台 fallback 和视觉预检；通过后才可进入 `ready`。该预检不关闭 M4-U5，published 投影仍为空。

...
- 阶段：M1 已冻结为工程参考；M2 与 M3 已完成并形成历史基线 `f258227`、`c606f5`。M4-U1/U2 的历史基线为 `5f327b6`，M4-U3 为 `e94eaca`，首个纵切片 release-readiness 为 `eb6e20c`；后续检查点包含 `a3194d0`、`d60691a`、CJK 字体提交 `9434a76` 与 M4-U5/功能页提交 `e2893d1`。经 2026-09-02 项目总检与范围重定界，M4 本地产品实现由 Project owner 提交为 `3983bee`；M5-U1–U3、U4 账户准备快照与第二 Collection 决策包随后提交为 `3fa46d5`。Project owner 后续确认 007 推荐、Liaozhai 任笃行 2016 主/张友鹤 2011 对校路线及《促织》青柯亭单见证路线，并逐次授权四篇研究、证据闭合、最小 draft Entry owner 及证据已闭合子集的 Source/Claim/Terminology 物化；这些结果与 Buttondown 审核事实同步已形成本地检查点 `9914dd3`。Project owner 又批准 Ten Kings 单篇纵切片并授权本地提交；随后另行授权 Fighting Cricket 单见证纵切片，并在中文概要复核后确认内容符合预期、授权本地提交。两篇均已进入 `7c0cb3f`，该检查点随后由 Project owner 推送；Guide 批开始时三个本地 ref 对齐。Project owner 于 2026-09-05 又通过中文概要确认 Liaozhai Reading Guide 内容符合预期，并单独授权验证通过后本地提交；该纵切片已提交为 `119c01c`；Project owner 随后确认已 push。009、Painted Skin、010 文案修订、聚焦双语确认与当前 R1 重建随后由 Project owner 提交为 `f81b49f`；Public Beta 路线合同又由 Project owner 提交为 `96da8db`。2026-09-07 本次五份视觉 brief 批开始前，HEAD、`main` 与本地 `origin/main` tracking ref 均为 `96da8db52cc384e0dc77164288d3bce16383faf0`，工作树干净；本次未 fetch，tracking ref 不单独证明服务器端状态。
- 当前实现：默认 review 构建按精确清单生成 14 个 `noindex, nofollow` 页面；8 个 Hero family 在 9 个所属页面合计引用 112 个 AVIF/WebP 输出。全站 Footer 恰有一个 disabled/inactive Newsletter，六个 Entry 在 Sources 与继续阅读路径后各有一个 disabled/inactive Reader Request；M5-U2 的三个 `src/services/` 仍只提供 strict provider-neutral 合同与零网络 Fake，仓库没有 form、供应商 action/link、远程脚本、配置、存储或真实 transport。当前内容 inventory 为 6 篇 Entry（2 `editorial-review` + 4 `draft`）、2 个 Collection（1 `editorial-review` + 1 `draft`）、14 份 Source、25 份 Claim 与 6 份 Terminology；published Entry / Collection 仍为 0/0。四篇新增 draft 和 Liaozhai Collection 已绑定各自 approved/current Hero；`relatedEntryIds`、Liaozhai Featured、内容状态与 M4 public SEO/artifact 接线仍未改变。
- 编辑/视觉状态：两篇首发 Entry 与 Chinese Underworld Collection 的 `editorial-review` 形态已获 Project owner 阶段性接受；四篇新增 draft 的英语单元、中文含义与两条 `contentNote` 已完成既有双语确认，六份 Terminology 仍为 `bilingual-approved`。012 的五份 Hero-only brief 已由 Codex 按 Project owner 英文审核委托确认；22 张候选收敛为五组 responsive 配对后，Project owner 明确批准全部五组并确认当前账户由其合法管理或已获组织授权，授权本批项目生成图保存、编辑和公开使用。五组现各有两份 exact-canvas master、production record、approved/current manifest、repository source 与 versionless 内容绑定；Codex 已完成成品视觉、文化、无障碍和语言终审。Liaozhai Collection 仍无 Featured，四篇 Entry 与 Liaozhai Collection 均保持 `draft`。
- 当前验证：2026-09-07 五组 Hero 数据闭环的定向检查已通过 8 个测试文件/38 项测试，覆盖 Schema、production lineage、metadata/hash、owner/brief/Claim 图、current resolver、内容 Hero 外键、21 个 local master 与精确 inventory。非默认 `visual:build:check` 已实际生成并解码 21 个 local master、17 份 current responsive rendition 对应的 120 个 AVIF/WebP 输出；最终完整 `pnpm run check` 通过 Prettier、ESLint、25 个测试文件/369 项测试、Astro 81 文件零诊断、14 页 build/output verifier、112 个 Hero、10 个 hash-locked WOFF2、零 XML 与零客户端 JavaScript。此前 14 页与 42 个 Hero 输出只保留为 brief 阶段历史证据。新增五组画布已做成品级视觉终审，但新增页面仍未做真实浏览器、键盘、200% 缩放、媒体偏好、故障注入或支持平台验收。
- 视觉/字体状态：M1 原型视觉皮肤仍未获批准；M4-U2 页面方向与此前 system fallback 三档视口已确认。当前 8 个 Hero family 的生产链均已闭合；4 份英文与 6 份 CJK WOFF2 静态门禁已通过。新的 noindex 样张覆盖 display/story/body、拼音/标点与 DESIGN 冻结混排行、SC/TC 400/500/600、65/36 个 required code points 与 `测`/`測` fallback-only probe；最终 HTML/CSS oracle 与 2026-09-02 三档浏览器矩阵已锁定生产 pinyin 语言、现有 token stack、权重/style、CJK inline boundary、20/20 可见样本及 10 个实际观测字体资源。Project owner 当时检查 M4 的 8 个页面路由并明确回复“这些页面通过”，该结论只关闭 2026-09-02 的 M4 页面样张，不覆盖 Privacy、四个新增 Entry、Liaozhai Collection 或本批新 Hero。当前控制面仍不能读取实际 fallback face，也不能可靠派发真实 Tab/Enter/Space、制造 200% zoom、启用 reduced motion、禁用 JavaScript、阻断字体/图片或取得受控 LCP/CLS；macOS/iOS/Android fallback 仍未闭合。这些项目明确保留为 M6 最终 public artifact 与受保护预览的发布候选 QA，不写成已通过，也不阻塞 M4 本地实现收口或自动提升内容状态。
...
- 合集接线：2026-09-05 完成两个 3 篇阅读路径；阴间按指南 → 十王 → 钟馗，聊斋按导读 → 画皮 → 促织。012 又完成四篇 Entry 与 Liaozhai Collection 的五组 Hero 资产闭环和内容绑定。状态仍为阴间 editorial-review、聊斋 draft，published 0/0；`relatedEntryIds`、Liaozhai Featured 和内容状态尚未审核。目标读者反馈为 0，按 011 留到 Public Beta 上线后。见 [009](docs/requirements/009-collection-reading-paths.md) 与 [012](docs/requirements/012-five-hero-visual-briefs.md)。
...
3. **M6 内容与 public artifact assembly**：Project owner 已确认 Liaozhai 第二 Collection 方向、3+3 分配、公开顺序、Zhong Kui Featured 与《促织》保留；两个合集路径已按 009 接线。四个 draft Entry 均有证据受限草稿，当前全库为 14 Source / 25 Claim / 6 Terminology。012 的五组 Hero 已完成 brief、候选/权利、exact-canvas、production lineage、approved/current manifest、repository source、内容绑定和资产级五审。下一内容停点是四篇 `relatedEntryIds`、Liaozhai Featured 与 Entry/Collection 状态审核；随后才进入稳定 HTTPS origin 与 public artifact assembly。R2 仍未通过，但不再排在这些工作之前。
...
| --- | --- | --- | --- | --- | --- |
| 本地应用、文档与原型工作区 | `F:\codex-project\mythic-china` | M4 基线为 `3983bee`；M5-U1–U3、U4 准备快照与 007 基线为 `3fa46d5`；四篇研究与证据最小物化检查点为 `9914dd3`；Ten Kings 与 Fighting Cricket 纵切片已进入 `7c0cb3f`，Liaozhai Reading Guide 检查点 `119c01c` 已由 Project owner 确认推送；当前 012 修改位于 `96da8db` 基线之上且尚未执行 Git 写操作 | 固定 Node、pnpm、CJK 生产、构建、验证与 Git 命令见 `DEV_WORKFLOW.md` | 6 Entry / 2 Collection / 14 Source / 25 Claim / 6 Terminology、21 个 local master、21 份 repository source、9 份 production record、12 份 manifest、三个 provider-neutral service/零网络 Fake、14 页 inert review UI 与 Privacy；四篇 draft 与 Liaozhai Collection 已绑定 approved/current Hero，外部 Buttondown/Tally 准备事实仍未接入仓库 | M4 review/public 纯门禁、M5-U2 内存合同、M5-U3 静态 UI/output 验证，以及 007–012 的内容、关系路径和五组 Hero 资产闭环；不含真实联调、`relatedEntryIds`、Liaozhai Featured、状态提升、M6 public artifact assembly、最终 release-candidate QA、clean-source receipt、远端预览或发布 |

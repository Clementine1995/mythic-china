...
- 当前验证：固定本机 Node/Corepack 下完整 `pnpm run check` 通过 22 个测试文件/207 项测试，Astro 70 文件零诊断，默认 review 构建通过 7 页/42 个 Hero 图片/10 个 hash-locked WOFF2/零 XML/零客户端 JavaScript，并通过 CJK 生产来源、构建后 CSS、有效 glyph cmap、parse5 HTML5 实际树语言边界、真实 published 空状态及 review 候选架精确链接门禁；非默认 `visual:build:check` 的既有证据为 11 个 local master、7 份 current responsive rendition 与 50 个实际 AVIF/WebP 输出复核，本批未改视觉链，未重跑该非默认入口。
- 视觉/字体状态：M1 原型视觉皮肤仍未获批准；M4-U2 页面方向与此前 system fallback 三档视口已确认。Project owner 在 2026-09-01 本地 review 中确认总体页面观感，同时指出 Explore/Collections 缺少可点击内容、About 首屏过空及部分字号偏大；对应 review 候选架、About 四节扩写和三个功能页作用域排版已实现并完成自动/浏览器复核，Project owner 随后结束本轮复看并授权停止服务。Collection Hero 与 Guide 自有 Hero 的生产链均已闭合；4 份英文与 6 份 CJK WOFF2 静态门禁已通过。慢加载、真实键盘/200%/媒体偏好、性能、跨平台 fallback 及完整 M4-U5 最终视觉尚未批准。
...
- 工作名称：`Mythic China`；目录名与未来包名使用 `mythic-china`。当前权威日期为 2026-09-01。

## 当前接力顺序

1. **首个纵切片与 M4-U5 预检**：Collection/Guide Hero、双语术语、四条馆藏标题 locale、CJK 静态门禁、Windows/local 四条直达页基础矩阵及三个功能页 review 候选架/排版复核均已形成。下一步先由 Project owner 查看本次 Explore、Collections、About 调整，再补真实键盘、200% 缩放、禁用 JavaScript、reduced motion、慢字体、图片失败、性能、跨平台 fallback 和专用字体样张；通过后才可进入 `ready`。该预检不关闭 M4-U5，published 投影仍为空。
...
当前已有本地应用开发与自动验证环境，但没有运行中的本地服务。Project owner 完成本轮页面复看后授权停止固定 preview；PID `20084` 已结束，端口 4321 无监听。该次服务授权已消费，不形成后续可复用授权。Vercel 已被选为未来静态托管目标，但项目仍没有真实联调环境、数据库、Vercel 项目、稳定 production alias/hostname、站点 origin 或远端发布资源。

| 环境或运行角色 | 固定工作区 | 代码/制品身份 | 配置入口 | 数据与外部资源 | 允许用途 |
| --- | --- | --- | --- | --- | --- |
| 本地应用、文档与原型工作区 | `F:\codex-project\mythic-china` | `eb6e20c` 为首个纵切片 release-readiness 基线；其后的具名本地检查点包含 `a3194d0` 与 `d60691a`，精确 HEAD、工作树及 tracking ref 以 `DEV_WORKFLOW.md` 的执行前只读核查为准 | 固定 Node、pnpm、CJK 生产、构建、验证与 Git 命令见 `DEV_WORKFLOW.md` | 本地 `editorial-review` 内容、三份 approved brief、四份 production record、七份 manifest、11 份 repository source rendition、11 个 local master、4 份英文 WOFF2、6 份 CJK 派生 WOFF2，以及已分别绑定 Collection 与 Guide 的 approved/current Hero v1；无数据库或真实外部写入口 | M4-U4A 纯门禁、首个纵切片编辑/视觉/字体静态候选与 noindex review 回归；不含字体浏览器/U5 完整验收、deployable public build、M4-U4B、远端部署或发布 |

...
- 当前验证：Liaozhai Reading Guide 纵切片完成后，固定 Node/Corepack 的完整 `pnpm run check` 最终通过 Prettier、ESLint、25 个测试文件/321 项测试、Astro 81 个文件零诊断、13 页静态 review build 与 output verifier；实际输出仍为 42 Hero、10 个 hash-locked WOFF2、零 XML、零客户端 JavaScript。第一次执行在任何测试断言失败前遇到一次 Vitest worker 意外退出；没有修改文件或降低门禁，原命令重跑后完整通过。HTML5 DOM oracle 逐页验证 Newsletter、六个 Entry 的 Reader Request、Privacy、顺序/唯一性、disabled/inactive 状态、稳定 page ID、零 provider link/form/script 和零假成功。Fighting Cricket 验证时对既有 Source 可见 CJK 记录的最小修正及证据物化、Ten Kings、M5-U3 的更早验证结果保留为历史证据。2026-09-02 的 24 个 M4 实际视口组合、12 个 Hero art-direction 组合与 Project owner 当时 8 页判断也仍是历史证据，均不覆盖四个新增 draft 页面。新页面未做真实浏览器/键盘/缩放验收；非默认 `visual:build:check` 不适用，因为本批没有修改图片、manifest 或 master。
...
- 版本边界：2026-09-02 进入 M5 前只读复核确认 HEAD 与本地 `main` 为 `3983bee91ada4a286613ec702a8009a4f528af3f`，后续本地 `origin/main` tracking ref 由外部 push 对齐；Project owner 又把 M5 与 007 状态提交为 `3fa46d5f85c43a5278e15ca6b0630d724439acc9`。四篇研究、证据最小物化及相关状态同步提交为本地检查点 `9914dd304467118b2c32f7f8ac192cc00344fb92`；Ten Kings 内容纵切片随后进入 `a5a15a5c74f7a18a8c15e0cdd5ac1e44574151c2`，Fighting Cricket 纵切片在中文概要确认与提交授权后进入 `7c0cb3fbea2c6175f01790160fe2460bb658e0f8`。Project owner 随后推送该检查点；本批开始时 HEAD、`main` 与本地 `origin/main` tracking ref 对齐。Liaozhai Reading Guide 在 2026-09-05 获中文概要确认与独立本地提交授权后随本检查点进入当前本地 HEAD；本检查点未执行 fetch、push、建分支或 worktree。提交前的本地 `origin/main` tracking ref 仍为 `7c0cb3f`，不能替代服务器端只读复核；精确身份须按 `DEV_WORKFLOW.md` 在操作前复核。
- 工作名称：`Mythic China`；目录名与未来包名使用 `mythic-china`。当前权威日期为 2026-09-05。
...
| --- | --- | --- | --- | --- | --- |
| 本地应用、文档与原型工作区 | `F:\codex-project\mythic-china` | M4 基线为 `3983bee`；M5-U1–U3、U4 准备快照与 007 基线为 `3fa46d5`；四篇研究与证据最小物化检查点为 `9914dd3`；Ten Kings 与 Fighting Cricket 纵切片已进入 `7c0cb3f`，Project owner 推送后本批开始时本地 HEAD、`main` 与 tracking ref 对齐。Liaozhai Reading Guide 在中文概要确认与独立本地提交授权后随本检查点进入当前本地 HEAD；本检查点未 fetch 或 push，服务器端状态仍须另行复核 | 固定 Node、pnpm、CJK 生产、构建、验证与 Git 命令见 `DEV_WORKFLOW.md` | 6 Entry / 1 Collection / 14 Source / 19 Claim / 5 Terminology、既有资产/字体 inventory、三个 provider-neutral service/零网络 Fake、13 页 inert review UI 与 Privacy；Ten Kings、Fighting Cricket 与 Liaozhai Reading Guide 已有证据受限 draft，外部已有审核通过的 Buttondown 账户及 Tally Free 未发布草稿，但仓库无凭据、action/link、provider 配置、数据库或外部写入口 | M4 review/public 纯门禁、M5-U2 内存合同、M5-U3 静态 UI/output 验证，以及 007/008 方向、研究、证据最小物化与三篇单篇纵切片；不含真实联调、图片、Collection 新关系、状态提升、M6 public artifact assembly、最终 release-candidate QA、clean-source receipt、远端预览或发布 |
